...
$projectPath = Join-Path $repositoryRoot "apps\windows\tests\IptvSuite.SecretStoreSpike\IptvSuite.SecretStoreSpike.csproj"
$assemblyPath = Join-Path $repositoryRoot "apps\windows\tests\IptvSuite.SecretStoreSpike\bin\x64\Release\net10.0\IptvSuite.SecretStoreSpike.dll"
$maximumBuildNodes = 1
...
function Invoke-CheckedDotNet {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Secret-store spike failed; failure=$FailureCode."
    }
}

$previousCliUseMsbuildServer = $env:DOTNET_CLI_USE_MSBUILD_SERVER
$previousDisableNodeReuse = $env:MSBUILDDISABLENODEREUSE
$previousValidatedSdk = $env:IPTVSUITE_SPIKE_VALIDATED_SDK
$previousRunnerAssemblySha256 = $env:IPTVSUITE_SPIKE_RUNNER_ASSEMBLY_SHA256
$locationPushed = $false

try {
    $env:DOTNET_CLI_USE_MSBUILD_SERVER = "0"
    $env:MSBUILDDISABLENODEREUSE = "1"
    $env:IPTVSUITE_SPIKE_VALIDATED_SDK = $actualSdk
    ...
    if (-not (Test-Path -LiteralPath $assemblyPath -PathType Leaf)) {
        throw "Secret-store spike failed; failure=runner-assembly-missing."
    }
    ...
    if ($runnerAssemblySha256 -notmatch '^[0-9a-f]{64}$') {
        throw "Secret-store spike failed; failure=runner-assembly-hash-invalid."
    }

    $env:IPTVSUITE_SPIKE_RUNNER_ASSEMBLY_SHA256 = $runnerAssemblySha256
    ...
    if ($null -eq $previousValidatedSdk) {
        Remove-Item Env:IPTVSUITE_SPIKE_VALIDATED_SDK -ErrorAction SilentlyContinue
    }
    else {
        $env:IPTVSUITE_SPIKE_VALIDATED_SDK = $previousValidatedSdk
    }

    if ($null -eq $previousRunnerAssemblySha256) {
        Remove-Item Env:IPTVSUITE_SPIKE_RUNNER_ASSEMBLY_SHA256 -ErrorAction SilentlyContinue
    }
    else {
        $env:IPTVSUITE_SPIKE_RUNNER_ASSEMBLY_SHA256 = $previousRunnerAssemblySha256
    }
}
