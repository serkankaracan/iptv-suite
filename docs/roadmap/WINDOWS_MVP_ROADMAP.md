...
**Implementation status:** IN PROGRESS — bounded `ICatalogBrowser` source/category/search/page port'u, real SQLite query adapter'ı, 200 kayıtlık page sınırı, 250 ms debounce ve generation-bound stale-result suppression uygulanmıştır. WinUI composition root device-local `Catalog\v2\catalog.db` query adapter'ını kurar; source/category/search/paging, virtualized `ItemsStackPanel`, loading/empty/failure durumu ve UIA adları olan player'sız katalog ekranını derler. Active snapshot'taki exact encrypted logo tuple'ını çözen provider yalnız durable source ile aynı HTTPS origin'e gider; 512 KiB response/signature sınırı, dört eşzamanlı fetch, 128-entry FIFO memory cache, page cancellation ve stale-row suppression uygular. Ayrı nonpackable M9 harness'ı disposable package `LocalCache` altında 50.000 sentetik provider-key channel'ı gerçek SQLite v3 şemasına seed eder ve production query ile 200-item/50k ilk sayfayı doğrular; credential, protected locator veya network kullanmaz. Signed package smoke gerçek WinUI ağacında name/role + Tab focus sırasına ek olarak 50k status, ≤300 automation list item, 20 örnek input-call p95 ≤100 ms ve 240 PageUp/PageDown girdili kontrollü foreground scroll sırasında DWM composition timing proxy'siyle frame p95 ≤33,3 ms, dropped frame <%1 ve maximum interval ≤200 ms kontrollerini fail-closed çalıştıracak biçimde genişletilmiştir [S113]. DWM ölçümü sistem compositor proxy'sidir; app-specific ETW/UI-thread attribution veya genel cihaz matrisi değildir. Local shell elevated olmadığı için bu packaged yürütme henüz **UNVERIFIED**dır. Exact-SDK local gate architecture 24 + unit 162 + integration 106 = `292/292 × 2`, Debug/Release 0 warning/error, fixture/sentinel/canary PASS'tir. Clean commit `d3ebbdcf24bb842bbab8f39c3abbed47a085db65` 50k ×20 query Decision'ında first-page/category/search p95 değerlerini sırasıyla `7,704`/`8,352`/`22,490 ms`, adapter reopen+first-page p95 değerini `7,443 ms` ölçerek query `≤100 ms` ve cached component `≤500 ms` bütçelerini karşılamıştır. [Query Decision evidence](../quality/M9_CATALOG_QUERY_DECISION_EVIDENCE.md). Hosted data-bearing UIA/frame acceptance henüz kanıtlanmadı; M9 tamamlanmamıştır.
