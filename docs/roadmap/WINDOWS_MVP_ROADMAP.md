...
**Current milestone status — `HARDENING SOURCE DRIFT / HOSTED SBOM RENEWAL REQUIRED / BLOCKED, 2026-08-27`:** Güncel schema-v7 release-readiness validator'ı exact x64 mimari/import-surface, asset provenance, package inventory ve tarihsel SBOM/CVE ledger sözleşmelerini doğrular. Sonraki security/native hardening package-producing snapshot'ı değiştirdiği için intact SBOM kabulü current candidate'da `stale-reopen`, `currentAtEvaluation=false`, `effectiveClosedBlocker=None` olur; `SbomPending` yeniden açılır. Final-release CVE freshness geçerliyken `technicalBaselinePassed=false`, `releaseReady=false` ve exact 13 M15 blocker vardır; freshness sona ererse `CveReviewPending` ayrıca açılır. Beklenen source drift'i blocked evidence üretimini engellemez; ledger/schema/contract tahrifi, daha yakın package override ve iki geçiş arasındaki TOCTOU değişimi hard-invalid kalır. Yeni clean hardening commit'i hosted SBOM/CVE zinciriyle yenilenmeden M15 tamamlanmış sayılmaz. Önceki clean source/install-root, x64 disposition ve hosted producer checkpoint'leri tarihsel kanıt olarak korunur. [Teknik baseline ve exact blocker seti](../quality/M15_RELEASE_READINESS_BASELINE.md).
...
**Current milestone status — `HARDENING SOURCE DRIFT / HOSTED RENEWAL REQUIRED / BLOCKED; 7 M16 blocker açık, 2026-08-27`:** Schema-v1 RC aggregator, sabit ve bounded predecessor evidence girdilerini fail-closed doğrulayıp sanitized `result=blocked`, `m1ToM15AutomatedGateSetPassed=false`, `m16TechnicalGateSetPassed=false` ve `candidateReady=false` özeti üretir; schema v1 yapısal olarak release-ready sonuç veremez. Clean `da205bd194016815ab069a3513eff4500796584d` için [run `33009018937`](https://github.com/serkankaracan/iptv-suite/actions/runs/33009018937), dört exact final-artifact yüzeyini `0` finding ile ve sentetik end-to-end journey'yi iki deterministik `614/614` quality runında geçti. Final-artifact ledger'ı ile journey ledger'ı historical ve intact kalır; ancak sonraki security/native hardening producer/dependency closure'larını değiştirdiği için ikisi de current candidate açısından `stale-reopen`dır. Aynı hardening M15 package-producing snapshot'ını değiştirdiği için `SbomPending` de yeniden açılır. Aggregator bu beklenen drift'i hard error'a çevirmeden blocked evidence üretir; raw ledger/schema/artifact tutarsızlığı hard-invalid kalır. Final-release CVE freshness geçerliyken 13 M15 blocker'ı ile yedi M16 blocker açıktır; freshness sona ererse `CveReviewPending` de açılır ve sayılar 14/21 olur. Yeni hardening commit'i hosted quality/final-artifact/SBOM evidence ile yenilenmeden hiçbir teknik kapı kapanmış sayılmaz. Bu checkpoint M15/M16 completion, private-flight/WACK/Store, hukuk/signing, security acceptance, fiziksel cihaz, feature-freeze veya uzun-soak kabulü değildir. [Blocked baseline ve non-claim'ler](../quality/M16_RELEASE_CANDIDATE_BASELINE.md).
