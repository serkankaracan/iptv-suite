...
**Implementation/acceptance status — `PARTIAL VERIFIED, 2026-08-25`:** Commit `8d537010f64193f7bc9baf05bc3707fb31d5362f` için [run `32764149667`](https://github.com/serkankaracan/iptv-suite/actions/runs/32764149667) signed production package'ta 25 rapid switch ile fullscreen/focus/resize/minimize/state/active-close zincirini geçti. [Run `32785306860`](https://github.com/serkankaracan/iptv-suite/actions/runs/32785306860)/commit `33afcef741f518b439c9934b0f4035fd92662586` packaged kısa-run resource guard'ını `PlaybackResourceBudgetVerified=true` ile geçti. [Run `32792088083`](https://github.com/serkankaracan/iptv-suite/actions/runs/32792088083) production source-delete success yolunu geçti. Commit `45124f72cc62b21942790c39bd032242706af339` için [run `32796910469`](https://github.com/serkankaracan/iptv-suite/actions/runs/32796910469), aynı signed production entry point'inde cancel/dialog-close no-mutation, deterministic pending failure, relaunch admission block, exact katalog/configuration/tombstone/sibling korunumu, fault release, manual retry, active-playback drain, final katalog/DPAPI cleanup ve completed tombstone binding kontrollerinin tamamını geçti; short-run resource budget da `true` kaldı (`-2.805.760 / +6.062.080 byte / -9 / -6`, p95 `2228,312 ms`). Bu ayrı guard M10/M16 uzun-soak kabulünü değiştirmez. Fiziksel DPI/mixed-DPI multi-monitor, gerçek sleep/wake, audio endpoint ve Narrator `NOT RUN` kaldığından milestone tamamlanmış sayılmaz. [Kısmi kabul kanıtı](../quality/M12_COMPLETION_EVIDENCE.md).
...
**Implementation status — `UNWIRED ORCHESTRATION KERNEL CHECKPOINT IMPLEMENTED, 2026-08-25`:** Saf Application kernel'i policy'yi monotonic `TimeProvider`, bounded injected jitter ve safe attempt executor ile bağlar. Aynı aktif correlation duplicate çağrıları tek task/timestamp/budget üzerinde coalesce eder; farklı veya yeni chain önceki generation'ı senkron invalid edip iptal eder. Exact 30 saniyelik provider-aware deadline token'ı delay ve in-flight attempt'e birlikte ulaşır; exact deadline'daki başarı fail-closed `ReconnectExhausted` olur. `1 s / 2 s / 4 s + 0..250 ms`, en çok üç attempt ve en çok bir saniyelik elapsed-derived countdown tick'i değişmemiştir. Global gate replacement/manual yarışında aynı anda yalnız bir attempt executor çalıştırır. Cancel terminal snapshot'ı senkron üretir ve fake time sonradan ilerlese yeni dispatch olmaz. Manual retry yalnız terminal safe correlation için immediate fresh generation/budget açar; aktif double-call mevcut chain ile coalesce olur. Public snapshot yalnız safe correlation/phase/attempt/max/bounded remaining/terminal code taşır; URL, selection, provider metni, exception veya delegate taşımaz. Bu checkpoint hâlâ **unwired**'dır: `PlaybackSessionCoordinator`, `PlaybackState.Reconnecting`, UI countdown/cancel, source retirement, app close, native error mapping, offline hint ve packaged recovery/no-later-open kabulü sonraki checkpoint'lerdedir. Native playback bugün `BoundedHttpTransport` kullanmadığı için transport'un kendi fetch retry'ları outer reconnect attempt'iyle çarpılmamış; `Retry-After` fetch katmanında kalmıştır.

**Implementation status — `COORDINATOR-CAPABLE, PRODUCTION-DISABLED CHECKPOINT IMPLEMENTED, 2026-08-25`:** Explicit deterministic coordinator overload'u reconnect kernel'ini exact generation, correlation, logical session ve selection context'iyle bağlar; mevcut tek argümanlı MainWindow construction'ı reconnect-disabled bırakıldığı için product/UI davranışı bu checkpoint'te değişmez. Canonical bounded-transient failure tek chain'e coalesce olur. Exact eski physical session başarılı biçimde drain edilmeden reopen yapılmaz; her attempt fresh monoton physical ID kullanırken public logical session ID sabit kalır. Engine callback'leri ve command/control/track çağrıları exact logical↔physical binding ile filtrelenir; replacement intent, stale physical callback, terminal pre/post-dispatch ve Stop/Release yarışları yanlış oturuma dispatch veya orphan chain üretemez. Desired volume/mute/aspect `Play` öncesi restore edilir; attempt failure exact rollback yapar, deadline sonrası reddedilen late-success binding terminal observer'dan önce exact drain'e alınır ve orchestrator `Succeeded` yayımlamadan recovery görünmez. `Start`, `Stop`, matching source release ve `Dispose` cancellation/drain sınırıdır; non-matching source release değildir. `PlaybackState.Reconnecting` yalnız Application session snapshot'ında aktif safe progress taşır ve engine contract'ında reddedilir. Terminal context sonraki manual retry UI checkpoint'i için korunur. Production composition opt-in, countdown/cancel/manual retry UX, native failure mapping, offline hint ve packaged recovery/no-later-open acceptance henüz tamamlanmamıştır.

**Implementation status — `PRODUCTION RECONNECT UI CHECKPOINT IMPLEMENTED, 2026-08-25`:** MainWindow explicit policy, `TimeProvider.System` ve kriptografik bounded `0..250 ms` jitter ile reconnect'i production composition'da etkinleştirir. WinUI yalnız safe `Evaluating|Waiting|Attempting`, attempt/max ve live-region dışındaki bounded saniye countdown'unu gösterir; reconnect sırasında mevcut Stop yolu erişilebilir `Cancel reconnect` olur, channel replacement açık kalır. Terminal exact context'te yalnız canonical `Manual` hata retry düğmesini açar; `Never`, malformed/stale context, matching source retirement, replacement ve dispose fail-closed kalır. Manual admission zincirin tamamını beklemeden döner ve duplicate çağrı aynı chain'e coalesce olur. Full snapshot freshness ile observer exception/reentrant mutation guard'ları eski progress'in yeni state'i ezmesini engeller. Local exact-SDK locked restore, Debug/Release x64 build sıfır warning/error, unit `335/335`, integration `142/142` ve architecture `43/43` PASS'tir. Native failure mapping, offline hint ve signed packaged recovery/user-cancel `≤1 s`/no-later-open acceptance henüz tamamlanmamıştır.

**Implementation status — `SAFE NATIVE PHASE MAPPING CHECKPOINT IMPLEMENTED, 2026-08-25`:** Windows-native adapter exact aktif source context'inin daha önce `Playing|Paused` durumuna ulaştığını yalnız process-local Boolean ile hatırlar. Bu kanıt varken sonraki `Buffering|Playing|Paused → MediaFailed` canonical `StreamInterrupted/BoundedTransient`; ilk açılıştaki failure ve bütün `SourceFailed` yolları `PlaybackStartFailed/Manual` kalır. Adapter native `ErrorMessage`, `ExtendedErrorCode`, `HResult`, exception metni veya locator ayrıştırmaz; dolayısıyla 401/403/404/429/5xx/TLS/DNS exact attribution yalnız M5 fetch katmanında kanıtlıdır ve production `MediaSource` yolu için hâlâ **UNVERIFIED**dır. Stall/EOF watchdog, offline hint, localized actionable presenter ve signed packaged recovery/cancel/no-later-open kabulü sonraki M13 checkpoint'lerinde kalır.

**Implementation status — `PLAYABLE RECOVERY HANDSHAKE + CONTROLLED FAULT FIXTURE CHECKPOINT IMPLEMENTED, 2026-08-25`:** Reconnect attempt artık `Open`/`Play` dönüşünü veya yalnız `Buffering` callback'ini recovery saymaz; yalnız güncel generation'ın exact physical session'ından gelen `Playing|Paused` callback'i attempt-owned completion'ı başarıyla kapatır. Stop, replacement, matching source release, dispose, rollback ve exact total deadline pending completion'ı invalid eder; geç veya stale callback yeni logical session'ı canlandıramaz ve physical session drain edilir. Test-only loopback fixture'ın yalnız açıkça etkinleştirilen route'u için paced stream, exact active-response abort, subsequent-request hold, tek bounded reject, restore, disable ve bounded scalar accounting sağlanır. Abort/detach attribution'ı atomik `Active → AbortRequested → Terminal` sahipliğindedir; disabled fallback aktif controlled response terminal olmadan başlamaz. Hook etkin değilken static/HEAD/range davranışı değişmez. Local exact-SDK locked restore, Debug/Release x64 build `0` warning/error, unit `338/338`, integration `156/156` ve architecture `44/44` geçmiştir. Bu primitive henüz production-package harness'ına bağlı değildir; signed recovery, user cancel `≤1 s`, no-later-open ve single-session acceptance kanıtı açık kalır.

**Implementation status — `SAFE FAILURE PRESENTATION + UNWIRED WATCHDOG KERNEL CHECKPOINT IMPLEMENTED, 2026-08-25`:** Terminal playback hatası yalnız canonical `DomainError` üzerinden `en-US`/`tr-TR` resource'larına çevrilir; eksik veya code/retryability/resource-key bütünlüğü bozuk hata generic güvenli mesaja fail-closed düşer. Her terminal snapshot için kriptografik rastgele 128-bit opaque operation ID bir kez üretilir ve reference-identity freshness/cache sözleşmesi aynı failure redraw'ında kimliğin dönmesini önler. Windows connectivity sonucu yalnız çevrimdışı/çevrimiçi presentation hint'idir; reconnect admission, engine sonucu veya retry authority değildir. Saf Application watchdog startup ile post-playable rebuffer deadline'ını ayrı izler; tekrar eden `Opening|Buffering` deadline'ı uzatmaz, `Playing|Paused` exact pending deadline'ı kapatır. Desteklenen explicit süreler `100 ms..2 dk`; monotonic exact boundary her wake-up'ta yeniden doğrulanır, erken callback kalan süreyle re-arm olur, cancel/dispose timer sahipliğini owner lock dışında temizler ve terminal observer seti tek sefer immutable yayımlanır. Startup timeout `PlaybackStartFailed/Manual`, rebuffer timeout `StreamInterrupted/BoundedTransient`, scheduler/clock contract failure `DomainInvariantViolation/Never` üretir; raw native/URL/provider/exception taşımaz. Local exact-SDK locked restore, Debug/Release x64 build `0` warning/error, unit `349/349`, integration `156/156` ve architecture `45/45` geçmiştir. Watchdog bu checkpoint'te production engine'e **bağlı değildir**; stall/EOF runtime mapping ile signed packaged recovery, cancel `≤1 s`, no-later-open ve single-session acceptance açık kalır.

**Implementation status — `PRODUCTION STARTUP/REBUFFER WATCHDOG WIRING CHECKPOINT IMPLEMENTED, 2026-08-25`:** Windows native engine, kendi ömründe sahip olduğu watchdog'u production için exact `10 s` startup ve `5 s` post-playable rebuffer süreleriyle `TimeProvider.System` üzerinde çalıştırır. Accepted `Buffering|Playing|Paused` snapshot watchdog'a dış observer'lardan önce gider; Stop, replacement, native failure ve dispose exact physical session generation'ını iptal eder. Timeout callback'i background thread'de native nesneye dokunmaz: yalnız UI dispatcher'a post eder, orada exact active context/session/generation/source bağını yeniden doğrular, canonical failure'ı kurar, handler detach → source null → owned `MediaSource.Dispose` teardown'ını dener ve ancak sonra terminal state'i yayımlar. Dispatcher shutdown `false` veya recoverable throw yolu cleanup/terminal olmuş gibi davranmaz. Startup stall `PlaybackStartFailed/Manual`, rebuffer stall `StreamInterrupted/BoundedTransient`, scheduler failure `DomainInvariantViolation/Never` kalır. İki argümanlı engine construction ve native error-message/HRESULT/URL ayrıştırma yasağı değişmemiştir. Local exact-SDK full gate Debug/Release x64 build'leri `0` warning/error; unit `349`, integration `156`, architecture `46` olmak üzere `551/551 × 2` deterministik run, fixture hash eşliği, sentinel fail/recovery ve artifact canary kontrollerini geçmiştir. Typed live EOF ve signed package recovery/cancel `≤1 s`/no-later-open/single-session acceptance hâlâ açıktır.

**Implementation status — `TYPED LIVE EOF BOUNDARY CHECKPOINT IMPLEMENTED, 2026-08-25`:** Production Windows native engine exact `MediaPlayer` sender'ından gelen `MediaEnded` olayını immutable session source'u ile UI dispatcher'a taşır; callback yalnız current context, generation, physical session, source ve non-retired/non-disposing ownership yeniden doğrulanırsa işlenir. İlk playable durumdan önce EOF `PlaybackStartFailed/Manual`, daha önce `Playing|Paused` görülmüş current `Buffering|Playing|Paused` live session'da EOF `StreamInterrupted/BoundedTransient` olur. Stop/replacement/dispose callback yarışları fail-closed düşer; handler detach → source null → owned source dispose teardown sırası ve cleanup-before-notify hattı korunur. Duration/position/`None`, URL/locator veya native `ErrorMessage`/HRESULT üzerinden natural completion ya da hata çıkarımı yapılmaz; VOD hâlâ kapsam dışıdır. Local exact-SDK full gate Debug/Release x64 build'leri `0` warning/error; unit `349`, integration `156`, architecture `46` olmak üzere `551/551 × 2` deterministik run, fixture hash eşliği, sentinel fail/recovery ve artifact canary kontrollerini geçmiştir. Signed packaged recovery, user cancel `≤1 s`, no-later-open ve single-session acceptance hâlâ açıktır.
...
**Supplemental measurement-contract hardening — `PASS, 2026-08-27`:** Commit `32fd7f803dfa6b16de8635372b3ce80197d0595c`, cancellation süresinin loader hazırlığını kapsayan eski başlangıç sınırını exact `CancellationRequestToLoaderCompletion` ile düzeltti ve 20 örnekli query p95'in scheduler-tail duyarlılığını schema v3 ile sınırladı. Parser/import/cancellation `20` sample'da; query exact `5` non-authoritative full-query warm-up + `100` `authoritative-warm` sample'da, fixed `FirstPage → CategoryPage → Search → ReopenFirstVisible` sırası ve `nearest-rank-ceiling` estimator ile ölçülür. Eşikler değişmedi. Tek binding baseline/candidate çifti parser `69,9869/71,1438`, combined import `1.462,5378/1.468,4712`, cancellation `0,7250/0,5526`, search `27,1650/26,9656` ve reopen `10,3267/10,1470 ms` p95 ile `8/8 ≤ +%10 PASS`; en yüksek artış parser `%1,653` idi. Baseline/candidate SHA-256 `15d54f1a34c5403b772292e2ead26a6086cff03604dce3cddfc80430d51a24e3` / `1e694e408b7adc3cd4b9e399830ebd99fe9398b2a70896da3db4488173ecb6b1`. Bu checkpoint historical completion'ı koruyan repeatability/measurement-contract hardening'idir; yeni ürün-performansı iddiası değildir.
...
**Current milestone status — `HISTORICAL SUCCESSOR HOSTED SBOM/CVE PASS / CURRENT WORKTREE STALE / BLOCKED; 12 external M15 blocker açık, 2026-08-29`:** Schema-v7 release-readiness validator'ı exact x64 mimari/import-surface, asset provenance, package inventory ve SBOM/CVE ledger sözleşmelerini doğrular. Clean `507e8f2c429759acf98d1a18477c994985ee357c` için [SBOM producer run `#19` (`33255219751`)](https://github.com/serkankaracan/iptv-suite/actions/runs/33255219751), o exact `138` dosyalık package-producing snapshot'ı ve exact yedi-file contract'ı official+strict `PASS` package/SBOM artifact'ına bağladı. Exact accepted SBOM ledger hash'ine zincirlenen clean `cc8073b48a340aabb8e30b28fe427d89669402e0` üzerindeki [known-vulnerability run `#98` (`33255965631`)](https://github.com/serkankaracan/iptv-suite/actions/runs/33255965631), exact `4/0/4` audited restore ve `2 + 21 = 23` package graph için `0/0/0` bilinen vulnerability sonucu üretti. Bunlar exact commit'lerine ait tarihsel kanıtlardır; güncel production drift'i nedeniyle M15 SBOM/CVE zinciri `STALE / RENEWAL REQUIRED`dır. Bu teknik kanıt M15 completion, private-flight/WACK/Store, hukuk/signing veya genel “CVE-free” iddiası değildir. [Teknik baseline ve exact blocker seti](../quality/M15_RELEASE_READINESS_BASELINE.md).

Bu checkpoint statik technical drift guard'ına, exact hosted package install-root audit'ine ve x64-only release disposition'a sahiptir. ARM64 desteği eklenmemiştir; ayrı native acceptance'a kadar Windows MVP support setinin dışındadır. Import-surface sonucu source-controlled MSBuild girdileriyle sınırlıdır; dış command-line/global-machine import kanıtı değildir. Disposable development identity Store identity değildir; dar runtime audit clean-VM install/update/reset/uninstall matrisini veya bütün olası filesystem write yollarını, statik package inventory ise tek başına SBOM/legal/CVE kapanışını kanıtlamaz. Aşağıdaki acceptance kriterleri ve kalan release blocker'ları açık kalır; M15 completed değildir.

**Package-bound SBOM checkpoint — `HOSTED VERIFIED / TECHNICAL SBOM ACCEPTED, 2026-08-25`:** Exact pin'li `Microsoft.Sbom.DotNetTool 4.1.5`, signed application MSIX ile exact Windows App Runtime `x64` dependency MSIX'i için companion SPDX `2.2` release seti üretir. Exact nupkg/shim/extracted payload ve iki paket imzası fail-closed bağlanır; resmî dokunulmamış araç çıktısı doğrulandıktan sonra repository-owned sıkı zenginleştirilmiş doğrulama, iki MSIX identity/hash bağını, exact production component setini ve release-set ilişkilerini denetler. Gerçek araç ve iki sentetik MSIX ile local uçtan uca prova `PASS`, architecture suite `52/52 PASS`, full quality gate `569/569 × 2 PASS`tir. Clean commit `12b1e95e8c3df04c42482daa52bdabd81abe1701` için [run #226 (`32897767622`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32897767622) package job `97966018579` ile hosted signed-package üretim/upload zincirini geçti. Artifact ID `9582332831`, SHA-256 `342fad95524b3624de842889428d4e2921ef3a481d3e8dd0b13ace27d932f106`; iç SBOM/summary/`last-success.json` SHA-256 değerleri sırasıyla `97d7e4aebedffbaae95a2d4e36f01bf1efff79dfe34cf001e7c787d637bffd39`, `d1bc7587ad3b5cbca42c78baad4c49a44f54199826f404b6b0849cf18435c5cd` ve `63644f96edb507be86980fb983fa69feef116652ea9f03d29da5f600414c3b04`; exact file/component/package/relationship sayaçları `2/24/27/43`tür. SHA-256 `853f1c702b9acc5e500d232688a22322aaeb6c3ff3f497a2fff269abc83fb904` source-controlled acceptance ledger'ı, exact 111 dosyalık package-producing snapshot (`465b2a74eba4f6c45871d57e4e042772a5a30024ff7e45ac7b9563571f101d9d`) ve exact yedi contract girdisi (`2b9cfe5d859ed070c47e2e74591b5567a5a8bc3a2006d2a5d775428f8a54c9ce`) ile birlikte schema-v4 evidence'ta yalnız `SbomPending` blocker'ını kapatır; product/source/solution/contract ekleme, silme, içerik drift'i veya daha yakın central-package importu eski kabulü fail-closed geçersiz kılar.

**Known-vulnerability checkpoint — `HOSTED VERIFIED / TECHNICAL REVIEW ACCEPTED, 2026-08-26`:** Clean commit `2053f4099819b3bb19bb9dd3370d60f0161098f1` için [run #4 (`32912296486`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32912296486), job `98008739618` ve artifact `9586961516`, exact SDK `10.0.302` ile production leaf'in dört project/dört lockfile/23 package graph'ını `4/0/4` audited restore, sıfır suppression/override ve direct/transitive/toplam `0/0/0` bilinen vulnerability sonucu ile geçti. Acceptance ledger SHA-256 `8c60360ae0dac240ef801688a04472266dabd16bfb1069a841b365b61c89a197`; exact 16-file contract SHA-256 `6b09978b5ee3ffc4d14e09458724a3d18fd1d23c5ec9ab3134dd25bfc7e91ff3` ve package graph SHA-256 `760562b81e0097913e1daf4ec88c67596337dd6636ed6d88c8f645424dc50b6e` ile fail-closed bağlıdır. Validator-owned yedi günlük freshness geçerliyken yalnız `CveReviewPending` kapanır; süre dolunca blocker yeniden açılır. Bu genel “CVE-free”, perpetual freshness veya hukuk kabulü değildir. Schema-v4 evidence `releaseReady=false` ve 13 blocker taşır; M15 tamamlanmış değildir. [Checkpoint ayrıntısı](../quality/M15_RELEASE_READINESS_BASELINE.md).

**Pre-asset SBOM/CVE kabul yenilemesi — `HOSTED VERIFIED / HISTORICAL, 2026-08-26`:** DWM refresh-counter gap normalizasyonu ve exact single-refresh maximum korumasından sonra bağımsız [SBOM producer run `#1` (`32918708589`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32918708589), clean `e7564aa9382fb12b10869866811e75847d1f6f16` üzerinde signed package/SBOM zincirini geçti. SBOM acceptance ledger SHA-256 `9eaf890f027f3485e89851701cfbfe4676afb44be4a918f525290c58385d9a21`; dedicated workflow dahil exact yedi-file contract SHA-256 `e5324fafa743cd003af480ab2c521ae2181a306fd67704c221d905025a7fccae`dir. Buna exact ledger hash'iyle zincirlenen [CVE producer run `#10` (`32919649968`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32919649968), commit `15826e5ecad4a4b5737aa7bf39d28c62b0e71de7` üzerinde `4/0/4` audited restore ve `0/0/0` bilinen vulnerability sonucunu yeniden geçti. Bu tarihsel CVE ledger SHA-256 `01180e5c90816fda2f1712a2ecd5ed634399caee76d68b7ffeb47c178e2aa208`; freshness sonu `2026-09-02T01:38:42Z`dir. Önceki `#226` ve `#4` kayıtları ilk kabul tarihçesidir; bu run `#1`/`#10` çifti ise asset byte değişiminden önceki son accepted zincirdir. Threshold ve hukuk kapsamı değişmedi.

**Deterministic package asset provenance — `TECHNICAL CHAIN ACCEPTED / 12 BLOCKERS, 2026-08-26`:** Manifestin exact sekiz PNG/ICO asset'i source-controlled `eng/New-WindowsProductionAssets.ps1` tarifiyle dış source asset, üçüncü taraf girdi, font, metin veya marka girdisi olmadan byte-identical üretilir. Strict provenance ledger'ı exact path/dimension veya ICO frame seti/length/hash bağlarını ve canonical asset setini fail-closed doğrular. Clean commit `b78c6c0512a91c9ab1fd86e18f703856a45a4b53` için [SBOM producer run `#2` (`32923642659`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32923642659), job `98042026161` ve artifact `9590841342`, yeni 111-file package snapshot'ını `6b068e8ccb651159dc2bad754f370ba428c6c3087c01c6bc7e419e6095cfadcc` hash'iyle official+strict `PASS` doğruladı; acceptance ledger raw SHA-256 `5ad87061b0586ad5d84f8f38d244bb3e9131358e27905bffd1f8b2648259bb47`dir. Buna zincirlenen [fresh CVE run `#13` (`32924589536`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32924589536), commit `58e0b615d1c531a9c94583772d4d80a5740eb3e3` üzerinde exact `4/0/4` audit ve 23-package graph'ını `0/0/0` known-vulnerability sonucuyla geçti; fresh-through `2026-09-02T02:58:07Z`dir. O checkpoint'teki schema-v6 sonucu `technicalBaselinePassed=true`, `releaseReady=false` ve exact 12 blocker'dır; `AssetProvenancePending`, `SbomPending` ve `CveReviewPending` teknik olarak kapalıdır. Üretilen görseller yalnız geliştirme amaçlı nötr teknik placeholder'dır; public marka/logo onayı, copyright ownership, redistribution kararı, root `LICENSE`/`NOTICE`, Store/legal veya codec-IP kapanışı değildir. Sıfır bulgu genel “CVE-free” iddiası değildir ve M15 tamamlanmış sayılmaz.

**Development identity WACK preflight — `EXECUTED / TEST FAILED, 2026-08-26`:** Opt-in `run_wack=true` manual workflow yolu, mevcut signed disposable development MSIX'i exact installed package-full-name üzerinden sistem WACK aracına verir. `reset` → bounded `test`, strict XML/root status kontrolü ve ham report/stdout/stderr cleanup'ı fail-closed sözleşmeyle korunur. Sonuç `DevelopmentIdentityWackPreflightOnly`, `ClosedBlocker=None`, `ReleaseReady=false` olarak sınırlandırılmıştır; production identity/release signer/exact RC üzerinde final tekrar olmadan `WackPending` kapanmaz. Değişen contract, [SBOM run `#3` (`32928618825`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32928618825) ve buna exact SBOM-ledger hash'iyle zincirlenen [CVE run `#18` (`32929529931`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32929529931) ile official+strict yeniden kabul edilmiştir. [Hosted run `32932186491`](https://github.com/serkankaracan/iptv-suite/actions/runs/32932186491) attempt 1 WACK öncesi tekil katalog input-response bütçesinde fail olmuş, eşik değiştirilmeden yapılan tek bounded attempt 2 ise package/SBOM/short-resource aşamalarını geçip exact `appcert.exe test` çağrısında stable `WindowsWack:TestFailed` üretmiştir. Ham çıktılar doğru biçimde silinmiştir. Takip Actions değişikliği yalnız allowlist'li stable failure code'u ayrı sanitized artifact'a çıkarır; gate kırmızı kalır, `continue-on-error` yoktur. Local PS5.1 self-test ve architecture `54/54 PASS` sonrasında [hosted run `32934454845`](https://github.com/serkankaracan/iptv-suite/actions/runs/32934454845), locked/DPAPI işlerini geçirip WACK job'unu beklendiği gibi kırmızı tuttu. Sanitize preparation/upload geçti; artifact `9594709230` yalnız schema-v1 `FailureCode=TestFailed` özetini taşıdı. Failure-evidence yolu hosted **VERIFIED**dır; WACK teknik sonucu hâlâ fail ve blocker açıktır.
...
**HTTP compatibility successor etkisi:** ADR-008/ADR-009 ve M17–M19 production değişikliklerinden sonra package/SBOM/CVE zinciri ile üç M16 automated acceptance lane'i exact successor hosted kanıta yeniden bağlanmıştır. Bu tarihsel kayıtlar yalnız run `#338`in clean `4daf95c615981a3bed4970e71575791646a2ae7a` commit'i ve kaydedilen exact source/package closure'ları için geçerlidir; güncel drift ilgili kapıyı fail-closed `stale-reopen` yapmıştır. Local implementation veya test sonucu hosted provenance yerine geçmez.

### Amaç

Kapsamı dondurulmuş, geri alınabilir ve yayımlama kararına hazır Windows MVP adayı üretmek.

**Current milestone status — `HISTORICAL SUCCESSOR AUTOMATED ACCEPTANCE / CURRENT WORKTREE STALE / BLOCKED; 2026-08-29`:** Schema-v1 RC aggregator sabit ve bounded predecessor evidence girdilerini fail-closed doğrular. Successor hosted run `#338` (`33256589562`), clean `4daf95c615981a3bed4970e71575791646a2ae7a` üzerinde iki deterministik `736/736` quality turunu, exact `86/86` ArchitectureTests altkümesini, sentetik journey'yi, dört yüzeyli final-artifact taramasını ve aynı-run MSIX/SBOM bağını kendi exact closure'ı için geçti. Güncel production drift'i nedeniyle final-artifact, security/architecture ve synthetic-journey acceptance ledger'larının üçü de `STALE / RENEWAL REQUIRED`dır. `m16TechnicalGateSetPassed=false` ve `candidateReady=false` değişmez. Feature freeze ile release-operations kabulü iki recorded-decision; fiziksel cihaz/accessibility matrisi ile 24 saat soak iki operator-evidence olmak üzere exact dört M16 blocker ve exact M15 dış blocker'ları korunur. Bu checkpoint M15/M16 completion, private-flight/WACK/Store, hukuk/signing, penetration/SAST, fiziksel cihaz, feature-freeze veya uzun-soak kabulü değildir. [Blocked baseline ve non-claim'ler](../quality/M16_RELEASE_CANDIDATE_BASELINE.md).
...
**M17–M19 automated checkpoint — `LOCAL 86/86 + HOSTED SUCCESSOR PASS WITH RELEASE BLOCKERS, 2026-08-29`:** Exact SDK locked restore ile Debug/Release x64 solution build'leri `0 warning / 0 error`, unit suite `409/409` ve izinli sentetik HTTPS loopback dahil integration suite `230/230` PASS'tir. İlk local ArchitectureTests koşusundaki `84/86` sonuç yalnız stale M15 SBOM ve M16 final-artifact acceptance pinlerinden geliyordu; successor M15/M16 ledger yenilemeleri sonrasında aynı suite `86/86` PASS'tir. Hosted run `#338` ayrıca iki deterministik `736/736` quality turunu, signed MSIX journey/final-artifact zincirini ve exact `86/86` ArchitectureTests altkümesini doğruladı. PowerShell package-smoke parser ve retired UIA identifier taraması PASS'tir. Bu checkpoint accessibility/manual checklist'i, fiziksel cihaz matrisini, 24 saat soak'ı veya M17–M19 completion kararını kapatmaz.
...
IPTVnator'ın exact `29ca94aa4371e9a71e24ccac4c529bd62f6fabd5` snapshot'ı üzerinden yapılan salt-okunur feature/UI audit'i sonrasında; özgün adaptive workspace, personal library/dashboard, EPG/XMLTV, catch-up/radio/timeshift, gelişmiş player controls, Stalker/Ministra, metadata, download, recording ve authenticated LAN remote adayları M20–M36 olarak planlanmıştır. Ayrıntılı kapsam, dependency sırası, güvenlik/lisans sınırları ve acceptance kapıları [IPTVnator referanslı Windows özellik genişleme yol haritasında](./IPTVNATOR_REFERENCE_WINDOWS_ROADMAP.md) kayıtlıdır.

Bu kayıt implementation başlangıcı veya feature parity iddiası değildir. IPTVnator marka/logo/artwork'ü kullanılmaz; Electron/Angular kodu ya da native runtime'ı doğrudan taşınmaz. M17–M19 bu adayları önceden scaffold etmez ve M20 çalışması M15/M16 release blocker'larını kapatmaz.
