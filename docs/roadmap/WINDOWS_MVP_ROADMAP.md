...
**Deterministic package asset provenance — `SBOM ACCEPTED / CVE RE-ACCEPTANCE PENDING, 2026-08-26`:** Manifestin exact sekiz PNG/ICO asset'i artık source-controlled `eng/New-WindowsProductionAssets.ps1` tarifiyle dış source asset, üçüncü taraf girdi, font, metin veya marka girdisi olmadan byte-identical üretilir. Strict provenance ledger'ı exact path/dimension veya ICO frame seti/length/hash bağlarını ve canonical asset setini fail-closed doğrular. Clean commit `b78c6c0512a91c9ab1fd86e18f703856a45a4b53` için [SBOM producer run `#2` (`32923642659`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32923642659), job `98042026161` ve artifact `9590841342`, yeni 111-file package snapshot'ını `6b068e8ccb651159dc2bad754f370ba428c6c3087c01c6bc7e419e6095cfadcc` hash'iyle official+strict `PASS` doğruladı. `AssetProvenancePending` ve `SbomPending` teknik zincirde kapalıdır. Üretilen görseller yalnız geliştirme amaçlı nötr teknik placeholder'dır; public marka/logo onayı, copyright ownership, redistribution kararı, root `LICENSE`/`NOTICE`, Store/legal veya codec-IP kapanışı değildir. Önceki SBOM ledger'ına bağlı CVE kabulü stale olduğundan fresh zincirli CVE producer/acceptance beklenir; final 12-blocker/readiness sonucu henüz iddia edilmez.
