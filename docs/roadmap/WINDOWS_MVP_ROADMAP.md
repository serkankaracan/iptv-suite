...
Reddedilen bulk yerleşim için ayrı `IptvSuite.ProtectedCatalogSpike` adayı x64, nonpackable/nonpublishable ve test-only'dir; solution build'inde derlenir fakat normal quality/hosted workflow executable'ı çalıştırmaz, production graph/MSIX dışındadır ve package denylist'i aday assembly/artifact adlarını reddeder. Big-endian/footerless immutable source/snapshot container'ı fresh DEK + CurrentUser DPAPI wrap + per-record AES-256-GCM kullanır; pre-activation structural reopen/unwrap/en fazla 16 tag probe ile sınırlıdır. Adapter reopen aynı process'tedir ve staging progress'i commit sayısı değildir. `.\eng\Invoke-WindowsProtectedCatalogSpike.ps1 -Mode Smoke` ile exact SDK `10.0.302` dirty-worktree Release koşusu 0 warning/error, 1k × 1, 22 true validation Boolean'ı, 10 encrypted record sonrası cancellation için sıfır post-cancel encryption/activation/mutation/temp artifact, canary/cleanup PASS ve targeted architecture `14/14` sonucu üretmiştir. `20469` byte summary SHA-256 `9d52172e61416fa1c73206ec745bd72a5aa18f15844153b08026c4597494cb99` ve `decisionEligible=false` taşır. Aynı worktree'de ayrıca full gate 199/199 testi iki aynı `Passed` setiyle ve tüm fixture/sentinel/scanner/artifact-canary kontrolleriyle geçmiştir; summary `commitSha=null` taşır ve commit-bound değildir. Ağır 5k–50k × 20 `Decision` çalıştırılmadı; fresh explicit onay + clean worktree + `-AllowDecision` ister. Prototype production-ready veya tercih edilen single-transaction SQLite key/ciphertext/active-snapshot tasarımı değildir; M4 `IN PROGRESS`, ADR-003 `Proposed` kalır. Run number `#24`, yalnız docs-only commit `207455a54d2d7ac9b6b5c1ce8eb5e29bbee0c383` için UI'da 3/3 iş, üç artifact ve `9m02s` gösterir; aday execution kanıtı değildir. Ayrıntı: [M4 protected-catalog aday kaydı](../quality/M4_PROTECTED_CATALOG_CANDIDATE.md).

Run number `#25`, `test(windows): spike protected catalog layout` başlıklı commit `786151cef6c60de5e34009ebb6643ec804490ee9` için UI'da 3/3 yeşil iş, üç artifact ve `8m32s` gösterir. Normal workflow solution build'iyle adayı derlemiş; architecture isolation guard'larını ve production MSIX'teki `IptvSuite.ProtectedCatalogSpike*` denylist kontrolünü geçmiştir. Bu yalnız hosted compile/isolation/package-exclusion için workflow/UI düzeyinde **VERIFIED**dır: opt-in executable `Smoke`/`Decision` modunda çalıştırılmamış, candidate evidence yüklenmemiş ve exact hosted test sayısı ile artifact içerik/tam digest bağları bağımsız doğrulanmamıştır. Heavy `Decision` ve milestone acceptance pending; M4 `IN PROGRESS`, ADR-003 `Proposed` kalır.
...
- Bulk channel locator için test-only immutable protected-catalog adayının güvenlik/layout spike'ı; reddedilen DPAPI-per-record baseline ile aynı 5k/10k/20k/50k × 20 protokolünde fresh-onaylı comparative `Decision` benchmark'ı ve ayrı production SQLite transaction tasarımı incelemesi.
...
- Reviewed alternatif için 5k–50k duration/allocation/working-set/cancellation.
- Crash injection ve startup orphan reconciliation.

### Ana risk

Per-record DPAPI-per-locator'ın doğrulanmış bütçe aşımından sonra reviewed alternatifin de bütçeyi karşılamaması veya app identity/lifecycle'ın protected data'yı orphan etmesi.
