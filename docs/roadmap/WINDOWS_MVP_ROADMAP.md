...
**Current Xtream update — supersedes the Xtream-open sentence in the implementation history below, 2026-08-24:** Live provider-item binding'i authoritative source configuration owner/reference/endpoint, canonical provider key ve yalnız `Hls`/`MpegTs` container hint'iyle JIT çözülür; credential lease işlem sonunda sıfırlanır ve sonuç yalnız kısa ömürlü locator lease'idir. `/live/{username}/{password}/{providerItem}.{m3u8|ts}` biçimi formal/evrensel Xtream standardı değil, mevcut live-only entegrasyon için dar bir `UNVERIFIED` uyumluluk varsayımıdır; farklı provider route'ları fail-closed kalır. Production-package adapter smoke hâlâ açıktır; bu checkpoint gerçek provider uyumluluğu iddiası değildir.

**Current production-package acceptance update — implementation/local verification complete, hosted execution open, 2026-08-24:** Non-production harness production Remote-M3U import, catalog encryption ve DPAPI `CurrentUser` zinciriyle yalnız sentetik Tier A kanal seed eder; bounded loopback HTTPS fixture yalnız aggregate sayaçlı ticket üretir. Package smoke, mevcut 50k UIA kabulünü değiştirmeden ikinci production activation'da exact kanal seçimi ve `play → pause → resume → stop` safe-state zincirini doğrular; test/harness payload sızıntısını reddeder ve exact certificate/process/package cleanup uygular. Exact SDK locked restore, Debug/Release x64 build, architecture `35/35`, unit `171/171` ve integration `116/116` geçmiştir. Yönetici/hosted gerçek MSIX koşusu geçmeden M11 tamamlanmış sayılmaz.

**Implementation status:** `IN PROGRESS — lifecycle + controls + Remote-M3U JIT resolution + Windows-native adapter + packaged UI composition checkpoints, 2026-08-24`. Plain `net10.0` Application katmanında engine-neutral `IPlaybackEngine`, güvenli `SourceId + ChannelId` selection contract'ı, typed state/result/event yüzeyleri ve app-owned `PlaybackSessionCoordinator` uygulanmıştır. Coordinator monoton session ID, tek engine mutation gate'i, session-lifetime cancellation, A→B exact stop-before-open, stop-during-open, caller/internal cancellation ayrımı, stale/illegal callback suppression, failure rollback ve idempotent dispose sınırlarını uygular. İkinci checkpoint `0..100` volume, mute, `Fit`/`Fill`, session-bound opaque audio/subtitle track ordinal'ları, `64` kayıtlık immutable inventory ve açık selection capability'si ekler. Control/query işlemleri aynı session lifetime + engine gate'ini kullanır; stale sonuçlar bastırılır ve control failure tek başına sağlıklı playback'i terminal yapmaz. Coordinator, başarılı open sonrasında desired engine-wide control değerlerini play öncesinde exact yeni session'a yeniden uygular; adapter reset'i ile Application state'inin ayrışmasına izin vermez. Üçüncü checkpoint internal Infrastructure resolver ile yalnız active snapshot'taki exact `SourceId + ChannelId` remote stream binding'ini `ChannelStreamLocator` owner/purpose/AEAD zinciri üzerinden JIT açar; strict UTF-8 ve mevcut HTTPS locator invariant'larını tekrar doğrular, invalid lease'i hemen sıfırlar ve raw locator'ı Application/catalog/UI yüzeyine çıkarmaz. Dördüncü checkpoint internal Windows-native adapter ile resolver lease'ini yalnız `MediaSource` yaratımı boyunca tutar; Media Foundation ve surface mutasyonlarını UI dispatcher'da serialize eder, native callback'leri exact session/generation/source bağında bastırır ve source detach/dispose sahipliğini failure/stop/final dispose yollarında kapatır. Beşinci checkpoint embedded `MediaPlayerElement` surface'ini page'e ekler; kanal `ItemClick` yalnız opaque selection ile coordinator'ı çağırır, built-in native controls kapalı kalır ve custom play/pause/stop kontrolleri coordinator'ı bypass etmez, state metni current session ile UI dispatcher'da doğrulanır ve window-close playback async teardown tamamlanana kadar ertelenir. Xtream provider-item kolu URL biçimi tahmin edilmeden açıkça fail-closed kalır. Public playback API locator, `Uri`, protected lease, native/WinUI tipi, track label/native ID, exception/HRESULT veya diagnostic text taşımaz; interface varlığı Tier B multi-track/subtitle desteği kanıtı değildir ve M2 `FakePlayer` değiştirilmemiştir. Production-package adapter smoke ve Xtream playback construction contract'ı hâlâ açıktır.
