...
**Implementation status:** `COMPLETED — CANDIDATE NO-GO, 2026-08-21` — disposable x64 WinUI/MSIX scaffold, exact locked set ve production leakage guard'ları geçti. Ancak restored `VideoLAN.LibVLC.Windows 3.0.23.1` x64 payload'ındaki `libx26410b_plugin.dll`, upstream GPL ayırma kaydı ve eksik exact binary-to-source/notices zinciri nedeniyle “GPL plugin/binary sıfır” hard gate'i kapatılamadı. Candidate erken reddedildi; problemli DLL körlemesine çıkarılmadı ve gereksiz 8 saat soak “pass” sayılmadı. Tekrarlanabilir karar [M10 evidence](../quality/M10_PLAYBACK_CANDIDATE_DECISION.md) içindedir. ADR-002 `Rejected`, dar Windows-native Tier A fallback [ADR-007](../adr/ADR-007-windows-native-tier-a-playback-fallback.md) ile `Proposed` durumdadır; fallback acceptance geçmeden M11 başlamaz.
