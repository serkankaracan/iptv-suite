...
`IMPLEMENTED / HOSTED PENDING`: [Schema-10 cancellation/recovery checkpoint'i](../quality/M10_NATIVE_TIER_A_CANCELLATION_RECOVERY_EVIDENCE.md), envelope v5 short lane'de `CancellationProbeCount=1` ile `NetworkInterruptionCount=1` kontrollerini bağımsız tutar. Application-owned managed cancellation exact source detach/dispose ve hedef 1000 ms no-auto-restart observation; fresh recovery source-specific `OpenOperationCompleted` ve exact current-source position advance ile fail-closed bağlanır. Native API cancellation, OS audio/session quiescence, M13 reconnect veya completion sonucu değildir. Hosted packaged successor sonucu beklenir; ADR-007 `Proposed`, R15 `ACTIVE` ve M10 `IN PROGRESS` kalır.
