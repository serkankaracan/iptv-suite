...
`VERIFIED`: Run `32537541013` ilk attempt'i tam HLS transferinden sonra ilk `MediaOpened` timeout'u; bounded rerun ise 100 switch ve schema-9 cleanup tamamlandıktan sonra `5056,3179 ms` startup maximum ihlali verdi (`p95=2157,0052 ms`). İlk measured playback önüne bounded `MediaPlayerElement.Loaded` bariyeri ekleyen `55945b9` checkpoint'inin run'ı `32540016736`, readiness'i geçtiği halde yine beş HLS response/`940229` byte ve sıfır transport failure sonrası ilk `MediaOpenTimeout` verdi; dolayısıyla `Loaded` eksikliği tek başına neden değildir. Statik inceleme, live IPTV probe'unda Microsoft'un real-time/live-streaming konfigürasyonu olan `MediaPlayer.RealTimePlayback` değerinin açılmadığını doğruladı [S118]. `INFERENCE`: Default non-real-time pipeline hosted cold HLS outlier'ına katkı veriyor olabilir. Source bağlanmadan önce explicit real-time mode eklendi; bounded readiness/lifetime zinciri, startup bütçeleri, HLS-first ve timeout retry yasağı korunur. Yeni hipotez hosted yeniden doğrulama bekler; M10/ADR-007 durumu ve 8 saatlik acceptance izni değişmez.
