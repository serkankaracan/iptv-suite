...
**Current milestone status — `SUCCESSOR HOSTED SBOM/CVE TECHNICAL PASS / BLOCKED; 12 external M15 blocker açık, 2026-08-29`:** Güncel schema-v7 release-readiness validator'ı exact x64 mimari/import-surface, asset provenance, package inventory ve SBOM/CVE ledger sözleşmelerini doğrular. Clean `507e8f2c429759acf98d1a18477c994985ee357c` için [SBOM producer run `#19` (`33255219751`)](https://github.com/serkankaracan/iptv-suite/actions/runs/33255219751), current `138` dosyalık package-producing snapshot'ı ve exact yedi-file contract'ı official+strict `PASS` package/SBOM artifact'ına bağladı. Exact accepted SBOM ledger hash'ine zincirlenen clean `cc8073b48a340aabb8e30b28fe427d89669402e0` üzerindeki [known-vulnerability run `#98` (`33255965631`)](https://github.com/serkankaracan/iptv-suite/actions/runs/33255965631), exact `4/0/4` audited restore ve `2 + 21 = 23` package graph için `0/0/0` bilinen vulnerability sonucu üretti. Yedi günlük teknik freshness `2026-09-05T13:52:01Z`, ayrı final-release freshness sınırı inclusive `2026-08-30T13:52:01Z`dir. Bu sınır geçerliyken local sonuç `packageSbomAcceptance.result=accepted-current`, `technicalBaselinePassed=true`, `releaseReady=false` ve exact 12 external M15 blocker'dır; bir saniye sonrasında `CveReviewPending` yeniden açılır. Bu teknik yenileme M15 completion, private-flight/WACK/Store, hukuk/signing veya genel “CVE-free” iddiası değildir. [Teknik baseline ve exact blocker seti](../quality/M15_RELEASE_READINESS_BASELINE.md).
...
**HTTP compatibility successor etkisi:** ADR-008/ADR-009 ve M17–M19 production değişikliklerinden sonra package/SBOM/CVE zinciri ile üç M16 automated acceptance lane'i exact successor hosted kanıta yeniden bağlandı. Bu `current` kayıtlar yalnız run `#338`in clean `4daf95c615981a3bed4970e71575791646a2ae7a` commit'i ve kaydedilen exact source/package closure'ları için geçerlidir; yeni drift ilgili kapıyı fail-closed `stale-reopen` yapar. Local implementation veya test sonucu hosted provenance yerine geçmez.

### Amaç

Kapsamı dondurulmuş, geri alınabilir ve yayımlama kararına hazır Windows MVP adayı üretmek.

**Current milestone status — `SUCCESSOR AUTOMATED ACCEPTANCE CURRENT / BLOCKED; 2026-08-29`:** Schema-v1 RC aggregator sabit ve bounded predecessor evidence girdilerini fail-closed doğrular. Successor hosted run `#338` (`33256589562`), clean `4daf95c615981a3bed4970e71575791646a2ae7a` üzerinde iki deterministik `736/736` quality turunu, exact `86/86` ArchitectureTests altkümesini, sentetik journey'yi, dört yüzeyli final-artifact taramasını ve aynı-run MSIX/SBOM bağını geçti; final-artifact, security/architecture ve synthetic-journey acceptance ledger'larının üçü de current'tır. `m16TechnicalGateSetPassed=false` ve `candidateReady=false` değişmez. Feature freeze ile release-operations kabulü iki recorded-decision; fiziksel cihaz/accessibility matrisi ile 24 saat soak iki operator-evidence olmak üzere exact dört M16 blocker ve exact M15 dış blocker'ları korunur. Bu checkpoint M15/M16 completion, private-flight/WACK/Store, hukuk/signing, penetration/SAST, fiziksel cihaz, feature-freeze veya uzun-soak kabulü değildir. [Blocked baseline ve non-claim'ler](../quality/M16_RELEASE_CANDIDATE_BASELINE.md).
