...
`VERIFIED — independent-segments negative A/B; zero-timeline successor pending`: Commit `e517fa8` run #156 (`32559852397`), quality/package/DPAPI/inventory adımlarını ve 100 switch'i geçti; p95 `2156,3241 ms`, fakat ilk HLS maximum `5105,5754 ms` oldu. Event kendi wait'i içinde geldi; hard total `5000 ms` sınırı `105,5754 ms` aşıldı. Böylece independent-segments metadata'sı standards-correct fixture sözleşmesi olarak kalırken yeterli startup düzeltmesi olmadığı doğrulandı. Sıradaki bounded A/B, direct'te zaten kullanılan `-muxdelay 0 -muxpreload 0` seçenekleriyle HLS remux container zaman tabanını direct başlangıcına hizalar; generator PTS/DTS cadence ve H.264/AAC elementary-stream eşliğini fail-closed doğrular [S129]. Hosted etkisi `UNVERIFIED`; arbitrary positive-base HLS desteği, genel HLS uyumluluğu, M10 completion veya sekiz saat acceptance kanıtı değildir. M10 `IN PROGRESS`, ADR-007 `Proposed`, R15 `ACTIVE` kalır ve M11 başlamaz.
