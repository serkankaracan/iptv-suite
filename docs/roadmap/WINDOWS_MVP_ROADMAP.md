...
**Development identity WACK preflight — `IMPLEMENTED / EXECUTION PENDING, 2026-08-26`:** Opt-in `run_wack=true` manual workflow yolu, mevcut signed disposable development MSIX'i exact installed package-full-name üzerinden sistem WACK aracına verir. `reset` → bounded `test`, strict XML/root status kontrolü, ham report/stdout/stderr cleanup'ı ve yalnız sanitized summary upload'ı fail-closed sözleşmeyle korunur. Sonuç `DevelopmentIdentityWackPreflightOnly`, `ClosedBlocker=None`, `ReleaseReady=false` olarak sınırlandırılmıştır; production identity/release signer/exact RC üzerinde final tekrar olmadan `WackPending` kapanmaz. Değişen contract, [SBOM run `#3` (`32928618825`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32928618825) ve buna exact SBOM-ledger hash'iyle zincirlenen [CVE run `#18` (`32929529931`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32929529931) ile official+strict yeniden kabul edilmiştir; development WACK execution hâlâ beklenir.
