...
M4'ü açık tutan hard-gate'ler: source disable + in-flight operation drain + source-wide deletion + `DeletionPending` orchestration ve startup protected-record orphan reconciliation; owner kimliğinin durable metadata'dan authoritative biçimde taşınması; ikinci gerçek Windows user. Protected-catalog comparative benchmark'ı kapanmıştır ve aynı reddedilmiş per-record yerleşimin yeniden koşulması yeni gate değildir. Owner + reference + endpoint tuple swap ile resolved-origin policy M5; security-reviewed DPAPI-wrapped per-snapshot DEK + per-row AES-GCM + active snapshot'ın aynı SQLite transaction domain'indeki production implementasyonu, crash/recovery ve end-to-end 50k kanıtı M8; production PFN lifecycle/repair/identity ve Store kabulü M15 kapsamındadır. Path/reparse check-to-use yarışı, 2026-08-14 threat-model kararında yalnız yükseltilmemiş, kullanıcıya özel local store ve same-user compromise sınırında residual risk olarak **ACCEPTED**dır; paylaşılan/ağ store, elevation veya same-user adversary scope değişikliği kararı yeniden açar [S94]. Tamamlanıp caller'a verilmiş bir plaintext lease delete ile geriye dönük revoke edilmez; lifecycle koordinatörü yeni resolve'ları durdurup aktif operasyonların kapanmasını beklemelidir. Açık kanıtlar olmadan aşağıdaki milestone acceptance maddeleri `UNVERIFIED` kalır.
...
- M8 production SQLite transaction yerleşimi için 5k–50k end-to-end duration/allocation/working-set/cancellation ve crash/recovery.
