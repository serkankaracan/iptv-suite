...
Bu checkpoint statik technical drift guard'ına, exact hosted package install-root audit'ine ve x64-only release disposition'a sahiptir. ARM64 desteği eklenmemiştir; ayrı native acceptance'a kadar Windows MVP support setinin dışındadır. Import-surface sonucu source-controlled MSBuild girdileriyle sınırlıdır; dış command-line/global-machine import kanıtı değildir. Disposable development identity Store identity değildir; dar runtime audit clean-VM install/update/reset/uninstall matrisini veya bütün olası filesystem write yollarını, package inventory ise SBOM/legal/CVE kapanışını kanıtlamaz. Aşağıdaki acceptance kriterleri ve kalan release blocker'ları açık kalır; M15 completed değildir.
