...
| R08 | 50.000+ playlist parse/protection/persistence süresi veya memory bütçesini aşar | Orta | Yüksek | **VERIFIED local Decision:** per-record protected create p95 `151.447 s` ve allocation p95 `668.007 MiB`; full parse/normalize/index/persistence pipeline henüz ölçülmedi | File-per-record bulk locator düzenini kullanma; streaming parser, bounded batch, index/staging ve stage metrics ile reviewed alternatif üzerinde full 5k–50k pipeline benchmark'ı; [Decision evidence](../quality/M4_SECRET_STORE_DECISION_EVIDENCE.md) | Data/Performance / M7–M8, M14; **ACTIVE** |
...
| R13 | DPAPI/LocalCache lifecycle veya 50k protected locator tasarımı beklenen güvenlik/performansı vermez | Yüksek | Yüksek | **Tetiklendi / VERIFIED local Decision 2026-08-14:** 50k file-per-record create p95 `151.447 s`, birleşik normalize + protected persistence + index `3 s` bütçesinin `50.48` katı; allocation p95 `668.007 MiB`, bütün-import `150 MiB` bütçesinin `4.45` katı | Bulk channel locator için file-per-record DPAPI düzeni reddedildi (`INFERENCE`); düşük-cardinality source credential kullanımı korunabilir. DPAPI-wrapped per-source DEK + standart AEAD veya vetted encrypted DB için yeni security decision; lifecycle, wrong-user ve reconciliation gate'leri açık; [Decision evidence](../quality/M4_SECRET_STORE_DECISION_EVIDENCE.md) | Security/Data / M4, M8; **ACTIVE** |
