...
| R08 | 50.000+ playlist parse/protection/persistence süresi veya memory bütçesini aşar | Orta | Yüksek | **VERIFIED local Decision:** per-record protected create p95 `151.447 s` ve allocation p95 `668.007 MiB`; full parse/normalize/index/persistence pipeline henüz ölçülmedi | File-per-record bulk locator düzenini kullanma; test-only immutable-container adayında fresh approval'lı comparative 5k–50k ölçüm; sonra seçilen transactional production layout'ında streaming parser + bounded batch + index/staging ile full pipeline benchmark'ı; [candidate](../quality/M4_PROTECTED_CATALOG_CANDIDATE.md) | Data/Performance / M7–M8, M14; **ACTIVE** |
...
| R13 | DPAPI/LocalCache lifecycle veya 50k protected locator tasarımı beklenen güvenlik/performansı vermez | Yüksek | Yüksek | **Tetiklendi / VERIFIED local Decision 2026-08-14:** 50k file-per-record create p95 `151.447 s`, birleşik normalize + protected persistence + index `3 s` bütçesinin `50.48` katı; allocation p95 `668.007 MiB`, bütün-import `150 MiB` bütçesinin `4.45` katı | File-per-record bulk locator reddedildi; düşük-cardinality source secret DPAPI store'u korunur. Immutable-container yalnız test spike `GO`/production `NO-GO`; encrypted DB M4 `NO-GO`; production için aynı SQLite transaction domain'inde key-state/ciphertext/snapshot pointer, lifecycle/wrong-user/reconciliation/crash ve 50k kanıtı gerekir; [candidate](../quality/M4_PROTECTED_CATALOG_CANDIDATE.md) | Security/Data / M4, M8; **ACTIVE** |
...
| R28 | Container ile katalog metadata'sının cross-file aktivasyonu veya encrypted-DB dependency/lisans seçimi recovery/supply-chain açığı üretir | Yüksek | Yüksek | Container publish olup DB pointer eski kalır veya tersi; key'siz active generation; unsupported `e_sqlcipher`/unknown native binary; lisans/notice/procurement açığı | Immutable container production `NO-GO`; key-state/ciphertext/snapshot/active pointer aynı SQLite transaction domain'inde. SQLCipher Commercial yalnız conditional future candidate; Community/`e_sqlcipher`/SEE M4 `NO-GO`; exact package digest/signature/SBOM/license/MSIX/migration/crash review | Architecture/Security/Legal / M4, M8, M15; **ACTIVE** |
...
1. **M4/M8:** protected storage güvenlik, cross-file atomiklik ve fresh-approval'lı 50k comparative/end-to-end throughput.
