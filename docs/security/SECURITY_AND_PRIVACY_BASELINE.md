...
2026-08-13 M4 foundation; store-owned reference issuance, source/purpose/reference-bound DPAPI CurrentUser envelope, bounded input/record, owned-buffer zeroization, store-operation safe failure mapping ve central sanitizer'ı uygular. Güncel source-draft operation'ı strict UTF-8, versioned ve length-prefixed payload'ı `SecretStoreLimits` sınırında üretir; Xtream için full locator+username+password'ı, remote playlist için full locator'ı yalnız protected-store create call'ına verir ve temporary byte buffer'ı `finally` içinde sıfırlar. Composition root factory'yi packaged launch'ta tam bir kez çağırır, başarılı `LocalCache\ProtectedStore\v1` store'unu uygulama ömrü boyunca tutar, initialization başarısızsa pencereyi oluşturmadan fail-closed olur ve daha zayıf fallback store kullanmaz. Factory beklenen initialization hatalarını raw path/message/store taşımayan typed sonuca eşler; caller cancellation'ını korur. Güncel local exact-SDK gate; CRUD/update/restart, idempotent delete, cleanup/binding/canary matrisi, source-draft, composition ve temp-cap davranışları dahil architecture 12, unit 114 ve integration 27 olmak üzere 153/153 testi iki deterministik koşuda geçmiştir. Arbitrary mid-I/O cancellation/interleaving ve cross-process sıralama kanıtı yoktur. SafeEndpoint/origin equality M5; `ProtectedStoreDirectoryInitialized` alanını taşıyan hosted packaged artifact, installed package update/reset/uninstall, second-user, source-deletion reconciliation ve 50k layout kararı ise açık M4 hard-gate'tir.
