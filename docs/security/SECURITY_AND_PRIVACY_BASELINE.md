...
| Protected storage | Packaged create/read/update/delete + reset/uninstall/reinstall/repair; per-record bulk locator düzeni reddedildi; immutable-container yalnız comparative spike; production SQLite state machine ve reviewed alternatifin 50k/crash kanıtı açık | M4/M8 |
...
- Per-record DPAPI bulk channel-locator throughput'u ölçülüp 50k bütçesinde reddedildi; düşük-cardinality source credential kullanımı korunur. Test-only immutable-container implementation'ının exact-hash bağlı dirty-worktree 1k Smoke correctness/harness sonucu vardır; heavy comparative `Decision`, crash/power-loss ve production SQLite transaction layout'u açıktır.
- Fresh-key/no-resume nonce invariant'ı, AAD tamper/swap/replay, cross-file activation reddi, key rotation/deletion ve DB journal/WAL/SHM recovery matrisi.
