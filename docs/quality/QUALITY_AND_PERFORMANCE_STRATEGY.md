...
`HARDENING SOURCE DRIFT / HOSTED RENEWAL REQUIRED / BLOCKED; 7 M16 blocker açık, 2026-08-27`: Schema-v1 fail-closed RC aggregator, fixed sekiz predecessor evidence girdisini bounded biçimde doğrular ve exact M15 blocker'larını değiştirmeden taşır. Clean `da205bd194016815ab069a3513eff4500796584d` için hosted run `33009018937`, dört final-artifact yüzeyini ve sentetik end-to-end journey'yi geçti; quality artifact'ı exact iki `614/614` deterministik run taşır. Bu iki historical ledger intact olsa da sonraki security/native hardening source closure'ları değiştirdiği için current candidate'ta `stale-reopen`dır: `M16FinalArtifactCanaryScanPending` ve `M16SyntheticEndToEndJourneyPending` yeniden açılır. Aynı hardening package-producing snapshot'ı değiştirdiği için M15 SBOM kabulü de `stale-reopen` olur ve `SbomPending` yeniden açılır. Beklenen source drift blocked evidence üretimini durdurmaz; raw ledger/schema/artifact inconsistency hard-invalid kalır. Böylece yeni clean hardening commit'i hosted evidence üretebilir ve ledger'lar daha sonra exact metadata/hash ile yenilenebilir. `m1ToM15AutomatedGateSetPassed=false`, `m16TechnicalGateSetPassed=false` ve `candidateReady=false` kalır. Final-release CVE freshness geçerliyken 13 M15 ve yedi M16 blocker; freshness sona erdiğinde 14 M15 ve toplam 21 blocker vardır. Aggregator 24 saat/200 switch soak'ı çalıştırmaz ve kısa native sonucu onun yerine kullanmaz. [Exact sözleşme ve non-claim'ler](M16_RELEASE_CANDIDATE_BASELINE.md).

Acceptance ledger raw SHA-256 değeri `48c8fe9d886d2fc66991304f6a169a911c73433e2c9e5edf52c506a59e2fbac1`dir. Exact `39` producer-contract source'u için strict UTF-8/no-BOM text girdileri LF-normalized, binary `.ts` fixture ise raw byte olarak bağlanır; source-set SHA-256 değeri `3318bf8638903bd05f509e29d4d6281e945773d0833003e3379a266f7b9ae2bb`dir. Ledger veya bu canonical contract bağı drift ederse final-artifact kabulü fail-closed geçersizleşir ve kapı yeniden açılır.
...
| M15 | `HARDENING SOURCE DRIFT / HOSTED SBOM RENEWAL REQUIRED, 2026-08-27`: hosted SBOM ledger/contract zinciri intact kalır, ancak current package-producing source snapshot'ı değiştiği için schema-v7 `result=stale-reopen`, `currentAtEvaluation=false`, `technicalBaselinePassed=false` ve `releaseReady=false` üretir. Final-release CVE freshness geçerliyken `SbomPending` dahil exact 13 blocker açıktır; freshness sona ererse `CveReviewPending` ayrıca açılır. Raw ledger/contract/override/TOCTOU ihlalleri hard-invalid kalır. WACK/private Store, production identity/lifecycle, license/NOTICE/privacy/codec-IP hukuk/signing ve diğer kapılar açık; M15 tamamlanmış değildir. [Baseline](M15_RELEASE_READINESS_BASELINE.md) |
| M16 | `HARDENING SOURCE DRIFT / HOSTED RENEWAL REQUIRED / BLOCKED`: run `33009018937` final-artifact ve sentetik journey historical pass'lerini taşır; sonraki hardening closure drift'i iki ledger'ı da `stale-reopen` yapar. Final-artifact ve journey kapıları dahil yedi M16 blocker ile exact M15 blocker'ları korunur; `m16TechnicalGateSetPassed=false`, `candidateReady=false`. [Baseline](M16_RELEASE_CANDIDATE_BASELINE.md) |
