...
`PARTIAL VERIFIED, 2026-08-25`: Commit `8d537010f64193f7bc9baf05bc3707fb31d5362f` için run `32764149667` signed production package'ta 25 rapid switch'i p95 `2231,596 ms` ile ve fullscreen/focus/resize/minimize/state/active-close zincirini geçti. Commit `b10b6ceb56eaba51668169dca904524808f4517b` için run `32783701519` protected source-delete UI route'unun locked quality, signed package ve DPAPI sınırlarını yeşil tuttu; product delete E2E değildir. `IMPLEMENTED LOCALLY; HOSTED VERIFICATION PENDING`: ayrı packaged kısa-run guard'ı signed delta için private `≤+8 MiB`, working set `≤+16 MiB`, handle `≤+64`, thread `≤0` ve değişmeyen rapid-switch p95 `≤3000 ms` sınırlarını uygular. Bu guard leak-yokluğu veya M10/M16 uzun-soak kabulü değildir. Fiziksel DPI/mixed-DPI multi-monitor, gerçek sleep/wake, audio endpoint, Narrator ve product source-delete E2E `NOT RUN` kalır. [Exact sınır](M12_COMPLETION_EVIDENCE.md).
...
| M12 | `PARTIAL VERIFIED, 2026-08-25`: run `32764149667` playback UX zinciri PASS; run `32783701519` protected source-delete UI checkpoint'inde required gate PASS. Signed short-run resource guard (`+8 MiB / +16 MiB / +64 / 0`, p95 `3000 ms`) implemented locally, hosted verification pending; fiziksel DPI/multi-monitor/sleep/audio/Narrator ve product source-delete E2E `NOT RUN`. [Kanıt](M12_COMPLETION_EVIDENCE.md) |
