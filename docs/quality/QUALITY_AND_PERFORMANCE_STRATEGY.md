...
| M9 | IN PROGRESS: bounded source/category/search/page query, 250 ms debounce, stale-result suppression ve virtualized WinUI browse surface local `289/289 × 2` PASS; clean 50k ×20 Decision first/category/search p95 `7,704`/`8,352`/`22,490 ms`, reopen+first-page `7,443 ms` ile query/cached component bütçeleri PASS; frame/realized-container, logo cache ve packaged UIA/keyboard kanıtı açık. [Kanıt](M9_CATALOG_QUERY_DECISION_EVIDENCE.md) |
