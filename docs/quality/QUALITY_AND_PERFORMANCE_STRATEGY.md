...
| M17–M19 Xtream successor contract | Aynı suite'lerin typed-content successor vakaları | Bounded `get_account_info → no action → get_profile` account profile; endpoint-derived Live/Movie/Series ayrımı; string-only `name/title` fallback; missing Series season metadata + raw 100/5k hierarchy cap; UTF-8 BOM; bounded unique numeric-key collection; exact `{}`/`null`/`false` empty sentinel; last-good anti-empty activation; non-empty error/mixed object rejection; account/Live/Movies/Series safe-stage sonucu; explicit 10k/50k limit propagation | Gerçek provider/account; first-import/explicit-empty outage ayrımı; visible partial warning; HTTP credential repetition; 50k üzeri truncation/pagination ürün kararı; hosted successor ledger renewal |
...
`M14 SUPPLEMENTAL MEASUREMENT HARDENING — 2026-08-27`: Commit `32fd7f803dfa6b16de8635372b3ce80197d0595c`, cancellation başlangıcını exact request anına bağlayarak eski loader-preparation kapsamını kaldırdı ve 20 örnekli query p95 tail duyarlılığını schema v3 ile sınırladı. Parser/import/cancellation `20` sample'da kalır; query exact `5` non-authoritative full-query warm-up + `100` `authoritative-warm` sample, fixed `FirstPage → CategoryPage → Search → ReopenFirstVisible` sırası ve `nearest-rank-ceiling` estimator kullanır. Eşikler değişmedi. Tek binding baseline/candidate çifti parser `69,9869/71,1438`, combined import `1.462,5378/1.468,4712`, cancellation `0,7250/0,5526`, search `27,1650/26,9656` ve reopen `10,3267/10,1470 ms` p95 ile `8/8 ≤ +%10 PASS`; en yüksek artış parser `%1,653` idi. Baseline/candidate SHA-256 `15d54f1a34c5403b772292e2ead26a6086cff03604dce3cddfc80430d51a24e3` / `1e694e408b7adc3cd4b9e399830ebd99fe9398b2a70896da3db4488173ecb6b1`. Bu yalnız repeatability/measurement-contract hardening'idir; yeni ürün-performansı iddiası değildir.
...
`M16 SUCCESSOR AUTOMATED ACCEPTANCE LEDGERS HISTORICAL / CURRENT WORKTREE STALE; 4 M16 blocker açık, 2026-08-29`: Schema-v1 fail-closed RC aggregator, fixed sekiz predecessor evidence girdisini bounded biçimde doğrular ve exact M15 blocker'larını değiştirmeden taşır. Successor hosted run `#338` (`33256589562`), clean `4daf95c615981a3bed4970e71575791646a2ae7a` üzerinde final-artifact, security/architecture ve synthetic journey kabullerini aynı quality/package zincirinde yeniledi. İki deterministik `736/736` quality turu, exact `86/86` ArchitectureTests altkümesi, signed MSIX lifecycle, DPAPI, dört yüzeyli final-artifact taraması ve required gate o exact closure için yeşildir. Sonraki production-source drift'i nedeniyle automated M16 ledger kapıları güncel worktree için `STALE / RENEWAL REQUIRED`dır; feature freeze ile release-operations kabulü iki recorded-decision; fiziksel cihaz/accessibility matrisi ile 24 saat soak iki operator-evidence olmak üzere dört M16 blocker açıktır. Raw ledger/schema/artifact inconsistency hard-invalid; security, TLS ve timeout eşikleri değişmemiştir. `m16TechnicalGateSetPassed=false` ve `candidateReady=false` kalır. Aggregator 24 saat/200 switch soak'ı çalıştırmaz ve kısa native sonucu onun yerine kullanmaz. [Exact sözleşme ve non-claim'ler](M16_RELEASE_CANDIDATE_BASELINE.md).

Final-artifact acceptance ledger raw SHA-256 değeri `eee1c4cebb23f84d2d871c98121b928f962d6aad0dd99b4ab2bf59edb75aa312`dır. Exact `39` producer-contract source'u için strict UTF-8/no-BOM text girdileri LF-normalized, binary `.ts` fixture ise raw byte olarak bağlanır; source-set SHA-256 değeri `3b096ed725b8e4294ba193198832b60b3b9698511477dde728ba4199bf1debb1`dir. Accepted historical package snapshot'ı exact `138` dosya / `92618f91a4bdfd4323843a65f4371f92f710dc5b07dbb0baeb8b5729d8b99e47` bağındadır. Security/architecture acceptance ledger raw SHA-256 `75eee910cd5796753b1c63072c172792b821ffc16b1d7eb3c176e2583c9b3438`; historical closure'ı `358` source / `7.830.014` canonical byte ve `a1c061c94bfaa27519eea78af15af334245678828205fe5e583262b882cac0bf` SHA-256 değeridir. Synthetic-journey acceptance ledger raw SHA-256 `28b105e0ba1d6b49abc89959bbef9b76649264ed6423eba209e51f868307c9b0`; historical closure'ı exact `148` source / `a871d3ebc6e4389bdc5f4b6e4448623c843141545ed3f94e55729ebd2f08464e` SHA-256 bağıdır. Güncel worktree bu canonical bağlardan drift ettiği için ilgili kabul fail-closed geçersizleşmiş ve kapı yeniden açılmıştır.
...
| M15 | `HISTORICAL SUCCESSOR TECHNICAL CHAIN / CURRENT WORKTREE STALE; EXTERNAL RELEASE BLOCKERS OPEN, 2026-08-29`: clean `507e8f2c429759acf98d1a18477c994985ee357c` üzerindeki SBOM run `#19` (`33255219751`) ve exact accepted SBOM ledger'ına zincirlenen clean `cc8073b48a340aabb8e30b28fe427d89669402e0` üzerindeki CVE run `#98` (`33255965631`) kendi exact closure'larında official+strict geçmiştir. Güncel production drift'i için successor SBOM/CVE yenilemesi gerekir. WACK/private Store, production identity/lifecycle, license/NOTICE/privacy/codec-IP hukuk/signing ve diğer kapılar açık; M15 tamamlanmış değildir. [Baseline](M15_RELEASE_READINESS_BASELINE.md) |
| M16 | `HISTORICAL SUCCESSOR AUTOMATED ACCEPTANCE / CURRENT WORKTREE STALE / BLOCKED, 2026-08-29`: clean `4daf95c615981a3bed4970e71575791646a2ae7a` üzerindeki run `#338` (`33256589562`), iki deterministik `736/736` quality turu, exact `86/86` ArchitectureTests, sentetik journey, dört yüzeyli final-artifact ve aynı-run MSIX/SBOM bağlarını kendi exact closure'ında geçti. Güncel production drift'i için üç acceptance ledger'ı `STALE / RENEWAL REQUIRED`dır. `m16TechnicalGateSetPassed=false`, `candidateReady=false`; feature freeze, fiziksel cihaz/accessibility matrisi, release-operations kabulü ve 24 saat soak olmak üzere exact dört M16 blocker ile exact M15 blocker'ları korunur. [Baseline](M16_RELEASE_CANDIDATE_BASELINE.md) |
