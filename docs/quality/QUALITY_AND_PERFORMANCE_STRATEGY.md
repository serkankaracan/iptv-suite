...
**Durum:** M2 quality infrastructure ile M3–M9 foundation/adapter/parser/persistence/query/UI gate'leri `COMPLETED`; M10 native Tier A fallback supply-chain rebaseline ve acceptance aktif
...
Güncel graph önce fail-closed exact package inventory/SBOM implementation ve hosted doğrulamasını; ardından ilgili kısa packaged/native, WACK, gerçek cihaz/HW-decode, surface/lifecycle ve soak matrisini yeniden geçmelidir. Teknik inventory sonucu repository kök `LICENSE`/`NOTICE`, uygulama asset provenance'i, corpus sahipliği/CC0 dağıtım hakkı veya codec/patent hukuk gate'ini kapatmaz. ADR-007 `Proposed`, M10 `IN PROGRESS`, R15 `ACTIVE` kalır ve M11 başlamaz.
...
`VERIFIED NEGATIVE — ilk sekiz saat successor-graph denemesi, 2026-08-22`: Commit `c9d5cd67607f97be9c1d49b68587f91c7515846c` üzerindeki gerçek Windows Client koşusu `100/100` switch, `7/7` interruption/recovery ve tutarlı `14369 completed + 49 I/O abort + 7 intentional 503 = 14425 mapped request` zincirinden sonra `ResourceBudgetExceeded` verdi. Kalıcı success evidence oluşmadı. Eski failure logu `ResourceSampleCount`, warm-up, byte, yüzde ve monotonic alanlarını göstermeden transient envelope cleanup'ına geçtiği için exact memory predicate'i `UNVERIFIED`dır. `droppedAfterCapacity=14393`, `14425 - 32` bounded trace omission sayısıdır; transport drop değildir. Diagnostic takip düzeltmesi tamamlanmış probe metriklerini korur ve safe aggregate resource değerlerini loglar; eşikleri değiştirmez. [Negatif kayıt](M10_NATIVE_TIER_A_ACCEPTANCE_SOAK_ATTEMPT.md) acceptance gate'ini kırmızı, R15'i aktif ve tekrar koşusunu zorunlu tutar.
...
`IMPLEMENTED LOCALLY; HOSTED ATTRIBUTION PENDING`: Davranış-korumalı first-HLS QPC transport window'u, packaged probe ile loopback TLS server olaylarını aynı sistemdeki `Stopwatch.GetTimestamp()`/`Frequency` tabanında bağlar [S130]. Transient envelope-v7 startup/source-open/`MediaOpened`/window-complete raw tick'lerini; en çok `32` öğelik in-memory trace ise accept, TLS auth, request header, response header, body write ve final flush tick'lerini taşır. High-resolution/frequency, monoton event, resource/method/range/body ve first-HLS truncation kontrolleri fail-closed'dur; yalnız bounded count/byte ve türetilmiş milisaniye attribution'ı loglanır. Raw QPC kalıcı kanıta girmez ve exact schema-10 success artifact'ı değişmez. Server response davranışı, player akışı, threshold, timer başlangıcı, HLS-first, retry ve cleanup aynıdır; hosted ölçüm gelmeden root cause veya düzeltme iddia edilmez.
...
| M10 | `CANDIDATE NO-GO; NATIVE FALLBACK REBASELINE IN PROGRESS, 2026-08-22`: exact LibVLC seti GPL-zero hard gate FAIL; ADR-002 rejected. Eski Windows App SDK `2.3.1 → WinUI 2.3.0` graph'ındaki native Tier A 100 switch/developer soak ve partial surface/network/source-detachment sonuçları tarihsel kaldı. Target Microsoft'un düzeltilmiş stable `2.4.0 → WinUI 2.3.6` graph'ına taşındı [S121][S122]. Fail-closed package inventory/SBOM implementation+hosted validation; ilgili kısa packaged/native rerun'lar, WACK, kalan surface/network/lifecycle/soak, gerçek cihaz/HW-decode, multi-monitor/display-change, audio-device, sleep/resume ve hukuk/provenance gate'leri açık. ADR-007 Proposed ve R15 ACTIVE; M11 başlamaz. [Candidate](M10_PLAYBACK_CANDIDATE_DECISION.md), [developer soak](M10_NATIVE_TIER_A_DEVELOPER_SOAK_EVIDENCE.md), [surface](M10_NATIVE_TIER_A_SURFACE_EVIDENCE.md), [network](M10_NATIVE_TIER_A_NETWORK_RECOVERY_EVIDENCE.md), [session lifecycle](M10_NATIVE_TIER_A_SESSION_LIFECYCLE_EVIDENCE.md) |
