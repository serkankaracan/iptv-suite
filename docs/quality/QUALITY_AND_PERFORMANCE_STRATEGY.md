...
Schema-9 cleanup contract'ı shared `Microsoft.WindowsAppRuntime.2` ownership'ini fail-closed sınırlar: pre-run exact package-full-name baseline'ındaki hiçbir kayıt kaybolamaz; post-run ekler exact Microsoft publisher/family, framework, `>=2.3.1.0` version ve izinli X64/X86 architecture ister, X86 ek aynı version'da exact X64 sibling olmadan reddedilir. Doğrulanmış shared ekler korunur ve `Remove-AppxPackage` çağrısı almaz; exact disposable test MSIX/app data/process/certificate/output cleanup'ı aynen kalır [S116]. Evidence `RuntimePackageBaselinePreserved`, `RuntimePackageGraphDisposition=ExactRestored|SharedAdditionsPreserved` ve bounded `RuntimePackageSharedAdditionCount` alanlarıyla bu iki sonucu ayırır. Bu kod/static contract kaydı hosted PASS değildir; 8 saatlik acceptance soak çalıştırılmamıştır.

`VERIFIED`: Run `32537541013` iki hosted attempt ile playback ve cleanup failure domain'lerini ayırdı. İlk attempt tam beş HLS yanıtı/`940229` byte sonrası ilk açılış timeout'u verdi; bounded rerun 100 switch ve schema-9 cleanup'ı tamamladı fakat startup maximum `5056,3179 ms` ile 5 saniye hard maximum'u geçti (`p95=2157,0052 ms`). Bounded `MediaPlayerElement.Loaded` bariyerli `55945b9` checkpoint'i de run `32540016736` içinde readiness'i geçtiği halde tam beş HLS response/`940229` byte ve sıfır transport failure sonrası ilk `MediaOpenTimeout` verdi; dolayısıyla `Loaded` eksikliği tek başına neden değildir. Live IPTV probe'unun `MediaPlayer.RealTimePlayback` konfigürasyonunu kullanmadığı statik olarak doğrulandı; Microsoft bu property'yi live streaming gibi real-time senaryolar için tanımlar [S118]. `INFERENCE`: Default non-real-time pipeline hosted cold HLS outlier'ına katkı veriyor olabilir. Probe source bağlanmadan önce real-time mode'u explicit açar; bounded readiness/lifetime zinciri de korunur. Ölçülen startup aralığı, `≤3000 ms` p95/`≤5000 ms` maximum, HLS-first ve yalnız explicit `MediaFailed` için bounded retry kuralları değişmez. Yeni hipotez henüz hosted PASS ile doğrulanmamıştır.
...
| M10 | `CANDIDATE NO-GO; NATIVE FALLBACK IN PROGRESS, 2026-08-21`: exact LibVLC seti GPL-zero hard gate FAIL; ADR-002 rejected. ADR-007 native Tier A fallback gerçek Windows Client'ta 100 switch + 120 dakika developer soak PASS; resize/minimize/restore/fullscreen, tek HTTP interruption/recovery ve 100/100 exact source-detachment alt kümeleri PARTIAL PASS. 8 saat, WACK, kalan surface/network, device/HW-decode, multi-monitor/display-change, audio-device ve sleep/resume matrisi açık. [Candidate](M10_PLAYBACK_CANDIDATE_DECISION.md), [developer soak](M10_NATIVE_TIER_A_DEVELOPER_SOAK_EVIDENCE.md), [surface](M10_NATIVE_TIER_A_SURFACE_EVIDENCE.md), [network](M10_NATIVE_TIER_A_NETWORK_RECOVERY_EVIDENCE.md), [session lifecycle](M10_NATIVE_TIER_A_SESSION_LIFECYCLE_EVIDENCE.md) |
