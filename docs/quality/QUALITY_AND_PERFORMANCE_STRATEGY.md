...
| Packaged lifecycle | `Invoke-WindowsPackageLifecycleSmoke.ps1` + ayrı nonpublishable MSIX | Aynı package version/PFN ve kullanıcı altında iki process launch; owner-v2 create/read, cross-owner negative, update/delete/post-delete, app-data canary scan ve exact cleanup | Production PFN, signed version update, reset/reinstall, second-user, crash/power-loss veya source reconciliation |
