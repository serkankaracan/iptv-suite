...
| M15 | `TECHNICAL CHAIN RENEWED / EXTERNAL RELEASE BLOCKERS OPEN, 2026-08-28`: clean `f6e8fdbb56644bff62ab261cb4725d1088e5124a` üzerindeki SBOM run `#15` (`33143245610`) ve exact accepted SBOM ledger'ına zincirlenen farklı clean `1c7ae00298414fb7707ad6097e151fc39aa38c03` üzerindeki CVE run `#88` (`33143833495`) official+strict current'tır. Final-release CVE freshness geçerliyken exact 12 external blocker açıktır; freshness sona ererse `CveReviewPending` ayrıca açılır. Raw ledger/contract/override/TOCTOU ihlalleri hard-invalid kalır. WACK/private Store, production identity/lifecycle, license/NOTICE/privacy/codec-IP hukuk/signing ve diğer kapılar açık; M15 tamamlanmış değildir. [Baseline](M15_RELEASE_READINESS_BASELINE.md) |
| M16 | `AUTOMATED ACCEPTANCE LEDGERS CURRENT / BLOCKED`: final-artifact ve security/architecture run `#327` (`33145434224`) ile exact `39`/`330` source closure'larında yenilendi; journey ayrı run `#321` (`33127559624`) ve exact `132`-source closure'ında current kalır. Feature freeze, fiziksel cihaz/accessibility matrisi, release-operations kabulü ve 24 saat soak olmak üzere exact dört M16 blocker ile exact M15 blocker'ları korunur; `m16TechnicalGateSetPassed=false`, `candidateReady=false`. [Baseline](M16_RELEASE_CANDIDATE_BASELINE.md) |
