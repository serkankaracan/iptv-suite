...
| Packaged launch | `Invoke-WindowsPackageSmoke.ps1` ile installed MSIX | Çalıştığı hostta sign, targeted production-payload inspection, install, AUMID visible launch, packaged `LocalCache\ProtectedStore\v1` directory initialization, normal close, exact cleanup | Secret CRUD, feature UI/UIA, update/reset, WACK/Store, provider/player/codec, ARM64, host/device matrisi |
...
Güncel source-draft kanıtı yalnız doğrudan await edilen çağrıda store-issued reference'ın başarılı sonuçla döndüğünü ve commit sonrası cancellation'ın yeniden gözlenmediğini kapsar. Caller abandonment/retry, duplicate create, process crash/OOM, metadata persistence ve orphan reconciliation kapsam dışıdır. Internal v1 decoder için encode→decode exact UTF-8 slice round-trip; wrong magic/kind/version; her truncation noktası; non-positive/overflow/inconsistent length; trailing byte; invalid UTF-8/control/whitespace ve exact Unicode-scalar sınırları test edilir. Decoder string/URI üretmez ve public resolver değildir; same-source semantic owner binding, M5 origin policy ve deletion/in-flight gate olmadan protected value consumer'ı açılmaz. Decoder dilimi dahil local exact-SDK gate architecture 12, unit 120 ve integration 27 olmak üzere 159/159 testi iki temiz koşuda aynı `Passed` setiyle; sentinel fail/recovery, fixture determinism, scanner CLI self-test ve artifact-file canary taramasını geçmiştir. Composition testi initialization failure'da pencere öncesi fail-closed davranışını da kapsar; bu 159-testlik decoder snapshot'ının hosted kanıtı henüz yoktur.
...
| M4 | IN PROGRESS: hosted fake + CurrentUser DPAPI foundation, source-draft protection, fail-closed retained packaged composition ve 1.024 temp-aday cap'i 153/153 iki-run PASS; `ProtectedStoreDirectoryInitialized=true`; internal decoder dahil güncel local gate 159/159 x2 PASS; packaged update/reset/uninstall, wrong-user, source deletion/record reconciliation ve 5k–50k Decision/ADR kararı açık |
