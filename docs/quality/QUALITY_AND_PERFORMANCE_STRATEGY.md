...
`PARTIAL VERIFIED, 2026-08-25`: Commit `8d537010f64193f7bc9baf05bc3707fb31d5362f` için run `32764149667` signed production package'ta 25 rapid switch ile fullscreen/focus/resize/minimize/state/active-close zincirini geçti. Run `32785306860`/commit `33afcef741f518b439c9934b0f4035fd92662586` ayrı packaged kısa-run guard'ını `PlaybackResourceBudgetVerified=true` ile geçti: private `-860.160 byte`, working set `+4.169.728 byte`, handle `-30`, thread `-7`, rapid-switch p95 `2229,342 ms`. Commit `4e96b27e1397c10df82b63531a3c3f227fe0f774` için run `32792088083`, production source-delete entry point'inde cancel/dialog-close no-mutation, active-playback drain, restart non-admission, exact katalog/DPAPI cleanup, tombstone completion ve 50k sibling retention sonuçlarının tamamını geçti; aynı run'ın short-run resource budget sonucu da `true`dur (`-1.478.656 / +6.258.688 byte / -4 / -6`, p95 `2233,135 ms`). Guard sınırları private `≤+8 MiB`, working set `≤+16 MiB`, handle `≤+64`, thread `≤0`, p95 `≤3000 ms`dir; leak-yokluğu veya M10/M16 uzun-soak kabulü değildir. Fiziksel DPI/mixed-DPI multi-monitor, gerçek sleep/wake, audio endpoint ve Narrator `NOT RUN` kalır. [Exact sınır](M12_COMPLETION_EVIDENCE.md).
...
| M12 | `PARTIAL VERIFIED, 2026-08-25`: run `32764149667` playback UX zinciri PASS; run `32785306860` signed short-run resource guard'ını PASS etti; run `32792088083` production source-delete cancel/close/confirm/relaunch, exact katalog/DPAPI cleanup, tombstone ve sibling-retention zincirini PASS etti. Fiziksel DPI/multi-monitor/sleep/audio/Narrator `NOT RUN`. [Kanıt](M12_COMPLETION_EVIDENCE.md) |
