...
`IMPLEMENTED LOCALLY; HOSTED A/B PENDING`: HLS remux'a, direct üretimde zaten bulunan `-muxdelay 0 -muxpreload 0` eklenerek aynı sentetik presentation'ın video PTS/DTS başlangıcı `1,480/1,400 → 0,080/0,000` saniye hizalandı [S129]. Generator dört segmentte iki saniyelik cadence'i, ilk audio timestamp parity'sini, sonlu timestamp'leri ve çıkarılmış H.264/AAC elementary-stream byte/hash eşliğini fail-closed doğrular. Dört TS toplamı `940000 → 978164` byte büyüdüğünden fixture kısaltılmamıştır. Hosted etki ve arbitrary positive-base HLS desteği `UNVERIFIED`dır; `≤3000/≤5000 ms`, HLS-first, retry, TLS/loopback, security ve cleanup sözleşmeleri değişmez.
