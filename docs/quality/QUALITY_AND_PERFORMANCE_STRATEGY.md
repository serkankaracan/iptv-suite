...
| M15 | `IN PROGRESS / BLOCKED, 2026-08-26`: clean `0236894f8cc8c6212305f5ba9c294e335f3d031e` schema-v1 static audit'te `technicalBaselinePassed=true`, `releaseReady=false`; exact 8 asset, 4 schema-v2 lockfile, 23 package adı ve tarihsel 16 blocker. Evidence `19.343` byte, SHA-256 `b41bad875b940958b52383051b215630fae22b8487ba3263f2237b78c9061895`; run #221 (`32873776770`) required zinciri PASS etti. Clean `5e60ca6bd60d72e35438a08b18f2d5fe4267f3e9` için run #223 attempt 2 (`32881949188`) exact install-root runtime audit'ini sıfır mutation ve pre/post equivalent `true` ile geçirdi; architecture `51/51`dir. Schema-v2 x64-only disposition `releaseArchitectures=[x64]`, ARM64 deferred ve `AppxBundle=Never` kararını version 1 source-controlled MSBuild import-surface denetimiyle fail-closed kilitledi; `Arm64ReleaseDecisionPending` kapandı ve o checkpoint'te 15 blocker kaldı. Run #226 (`32897767622`), commit `12b1e95e8c3df04c42482daa52bdabd81abe1701` üzerindeki package job `97966018579` ile signed package-bound SPDX `2.2` SBOM'u official+strict validation ve exact `2/24/27/43` sayaçlarıyla geçti; artifact `9582332831` digest'i `342fad95524b3624de842889428d4e2921ef3a481d3e8dd0b13ace27d932f106`dır. Schema-v3 acceptance ledger'ı yalnız `SbomPending` blocker'ını kapattı; `releaseReady=false` ve 14 blocker kaldı. WACK/private Store, production identity/lifecycle, license/NOTICE/CVE/privacy/codec-IP hukuk/signing ve diğer kapılar açık. [Baseline](M15_RELEASE_READINESS_BASELINE.md) |
| M16 | Full regression, 24h soak, secret/license/privacy gates |

`M15 CVE producer — LOCAL CONTRACT PASS / HOSTED ACCEPTANCE PENDING, 2026-08-26`: Ayrı `windows-cve-review.yml` lane'i exact SDK `10.0.302`, fresh bounded NuGet cache/home, locked ve `4/0/4` audited restore ile yalnız production Windows leaf'ini denetler. Filtresiz JSON v1 envanteri exact `2` direct + `21` transitive package'ı dört production lockfile'daki `23` package/content-hash graph'ına bağlar; filtreli JSON v1 sonucu exact `https://data.nuget.org/v3/index.json` source'unda `knownVulnerabilityCount=0` olmadan evidence üretmez. PS5.1 offline adversarial contract `PASS`tir. Bu yalnız producer checkpoint'idir; clean hosted artifact/acceptance ledger ve freshness/required-check kararı beklediğinden `CveReviewPending` ve toplam `14` blocker aynen açıktır. “0 bulgu” yalnız sorgu anındaki bilinen vulnerability verisi içindir, “CVE-free” iddiası değildir. Resmî dayanaklar (erişim `2026-08-26`): [.NET CLI](https://learn.microsoft.com/en-us/dotnet/core/tools/dotnet-package-list), [JSON v1 spec](https://github.com/NuGet/Home/wiki/%5BSpec%5D-Machine-readable-output-for-dotnet-list-package), [NuGet Audit](https://learn.microsoft.com/en-us/nuget/concepts/auditing-packages).
