...
`M16 AUTOMATED ACCEPTANCE LEDGERS CURRENT / BLOCKED; 4 M16 blocker açık, 2026-08-28`: Schema-v1 fail-closed RC aggregator, fixed sekiz predecessor evidence girdisini bounded biçimde doğrular ve exact M15 blocker'larını değiştirmeden taşır. Hosted ortak final-artifact/security/journey run `33127559624` kendi exact commit/artifact/source closure bağlarında current accepted evidence'dır. Ortak run `33127559624`, journey için exact `132`-source closure'ı; final-artifact/security için ise final `39`-source artifact ile `329`-source security closure'larını iki deterministik `631/631` quality turu, signed MSIX lifecycle, DPAPI ve required gate ile tamamen yeşil doğruladı. Automated M16 ledger kapıları current'tır; feature freeze ile release-operations kabulü iki recorded-decision; fiziksel cihaz/accessibility matrisi ile 24 saat soak iki operator-evidence olmak üzere dört M16 blocker açıktır. Raw ledger/schema/artifact inconsistency hard-invalid; security, TLS ve timeout eşikleri değişmemiştir. `m16TechnicalGateSetPassed=false` ve `candidateReady=false` kalır. Aggregator 24 saat/200 switch soak'ı çalıştırmaz ve kısa native sonucu onun yerine kullanmaz. [Exact sözleşme ve non-claim'ler](M16_RELEASE_CANDIDATE_BASELINE.md).

Final-artifact acceptance ledger raw SHA-256 değeri `89f87e92514cda2cbfc8d41f34a6d074d69871a6808086fb15876e0d0d95c05f`dır. Exact `39` producer-contract source'u için strict UTF-8/no-BOM text girdileri LF-normalized, binary `.ts` fixture ise raw byte olarak bağlanır; source-set SHA-256 değeri `f033e9721efd06184d5bc21c4fd215902b3c1e98fa3d5279cbd644e0d171f6bc`dir. Current package snapshot'ı exact `115` dosya / `5568fb8fc87f614392762501cb2a4b3be1a13487bb8cfab037ccaec579756810` bağındadır. Security/architecture acceptance ledger raw SHA-256 `fca033be76fb252ed2a4f3ac9dbf36dbcf0240b1f4eeb1dfd06210d79b7ca152`; current closure'ı `329` source / `7.204.158` canonical byte ve `7ee590883be2198547a336611370a629c5f20a7f16a5cdd6e3b9294fb626c031` SHA-256 değeridir. Ledger veya bu canonical bağlardan biri drift ederse ilgili kabul fail-closed geçersizleşir ve kapı yeniden açılır.
...
| M15 | `TECHNICAL CHAIN RENEWED / EXTERNAL RELEASE BLOCKERS OPEN, 2026-08-27`: package-producing snapshot, hosted SBOM ve CVE ledger/contract zinciri current'tır. Final-release CVE freshness geçerliyken exact 12 external blocker açıktır; freshness sona ererse `CveReviewPending` ayrıca açılır. Raw ledger/contract/override/TOCTOU ihlalleri hard-invalid kalır. WACK/private Store, production identity/lifecycle, license/NOTICE/privacy/codec-IP hukuk/signing ve diğer kapılar açık; M15 tamamlanmış değildir. [Baseline](M15_RELEASE_READINESS_BASELINE.md) |
| M16 | `AUTOMATED ACCEPTANCE LEDGERS CURRENT / BLOCKED`: ortak final-artifact/security/journey run `33127559624` current closure'larını hosted yeşil doğruladı. Final-artifact, security/architecture ve journey ledger'ları current'tır; feature freeze, fiziksel cihaz/accessibility matrisi, release-operations kabulü ve 24 saat soak olmak üzere exact dört M16 blocker ile exact M15 blocker'ları korunur; `m16TechnicalGateSetPassed=false`, `candidateReady=false`. [Baseline](M16_RELEASE_CANDIDATE_BASELINE.md) |
