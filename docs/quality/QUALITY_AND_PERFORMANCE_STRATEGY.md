...
`PARTIAL VERIFIED, 2026-08-25`: Commit `8d537010f64193f7bc9baf05bc3707fb31d5362f` için run `32764149667` signed production package'ta 25 rapid switch ile fullscreen/focus/resize/minimize/state/active-close zincirini geçti. Commit `b10b6ceb56eaba51668169dca904524808f4517b` için run `32783701519` protected source-delete UI route'unun locked quality, signed package ve DPAPI sınırlarını yeşil tuttu; product delete E2E değildir. Run `32785306860`/commit `33afcef741f518b439c9934b0f4035fd92662586` ayrı packaged kısa-run guard'ını `PlaybackResourceBudgetVerified=true` ile geçti: private `-860.160 byte`, working set `+4.169.728 byte`, handle `-30`, thread `-7`, rapid-switch p95 `2229,342 ms`. Guard sınırları private `≤+8 MiB`, working set `≤+16 MiB`, handle `≤+64`, thread `≤0`, p95 `≤3000 ms`dir; leak-yokluğu veya M10/M16 uzun-soak kabulü değildir. Fiziksel DPI/mixed-DPI multi-monitor, gerçek sleep/wake, audio endpoint, Narrator ve product source-delete E2E `NOT RUN` kalır. [Exact sınır](M12_COMPLETION_EVIDENCE.md).
...
| M12 | `PARTIAL VERIFIED, 2026-08-25`: run `32764149667` playback UX zinciri PASS; run `32783701519` protected source-delete UI checkpoint'inde required gate PASS; run `32785306860` signed short-run resource guard'ını PASS etti (`-860.160 / +4.169.728 byte / -30 / -7`, p95 `2229,342 ms`). Fiziksel DPI/multi-monitor/sleep/audio/Narrator ve product source-delete E2E `NOT RUN`. [Kanıt](M12_COMPLETION_EVIDENCE.md) |
