...
`IMPLEMENTED / HOSTED PENDING`: Schema-10/envelope-v5 kısa lane, `100` switch ve `0` soak içinde bağımsız bir sentetik network interruption ile bir application-owned managed cancellation probe'u birlikte fakat ayrı invariant'larla doğrular. Cancellation exact detach/dispose sonrasında hedef `1000 ms` boyunca null source ve automatic restart yokluğu; recovery ise fresh source-specific `OpenOperationCompleted` ile exact current-source position advance ister. [Contract/evidence kaydı](M10_NATIVE_TIER_A_CANCELLATION_RECOVERY_EVIDENCE.md) exact alan/parity/bütçe sınırlarını içerir. Bu OS audio/session quiescence, native API cancellation, M13 reconnect veya M10 completion kanıtı değildir; hosted packaged successor sonucu beklenir.
