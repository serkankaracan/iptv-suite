...
`M15 deterministic asset provenance — SBOM ACCEPTED / CVE RE-ACCEPTANCE PENDING, 2026-08-26`: Exact sekiz production-package PNG/ICO asset'i source-controlled ve dış girdisiz deterministic tariften yeniden üretildi; strict ledger path, dimension/ICO-frame, length, hash ve canonical asset-set bağını doğrular. Run `#2` (`32923642659`), commit `b78c6c0512a91c9ab1fd86e18f703856a45a4b53`, job `98042026161` ve artifact `9590841342` ile yeni package snapshot'ını official+strict `PASS` doğruladı; `AssetProvenancePending` ve `SbomPending` teknik olarak kapandı. Asset'ler yalnız nötr development placeholder'dır; marka/logo onayı, copyright ownership, redistribution, root `LICENSE`/`NOTICE`, Store/legal veya codec-IP kabulü değildir. Önceki SBOM ledger'ına bağlı CVE kabulü stale olduğundan fresh zincirli CVE producer/acceptance beklenir; schema-v5 için final 12-blocker/readiness sonucu henüz kaydedilmez.
