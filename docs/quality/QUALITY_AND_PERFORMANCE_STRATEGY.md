...
| Packaged lifecycle | `Invoke-WindowsPackageLifecycleSmoke.ps1` + ayrı nonpublishable MSIX | Baseline `0.0.1.0` create/duplicate-create; same-family/signer `0.0.2.0` normal update; updated-process owner-v2 read, cross-owner negative, update/delete/post-delete; post-update owned-surface scan ve exact cleanup | Yeni update lane'i için hosted execution, production PFN, Windows-managed package hive'ları, reset/reinstall, second-user, crash/power-loss veya source reconciliation |
