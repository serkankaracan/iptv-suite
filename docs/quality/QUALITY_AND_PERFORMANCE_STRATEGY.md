...
`VERIFIED`: Run `32537541013` iki hosted attempt ile playback ve cleanup failure domain'lerini ayırdı. İlk attempt tam beş HLS yanıtı/`940229` byte sonrası ilk açılış timeout'u verdi; bounded rerun 100 switch ve schema-9 cleanup'ı tamamladı fakat startup maximum `5056,3179 ms` ile 5 saniye hard maximum'u geçti (`p95=2157,0052 ms`). Kodun ilk ölçümü `MediaPlayerElement.Loaded` öncesinde başlattığı da statik olarak doğrulandı. `INFERENCE`: Interaction-ready `Loaded` sınırının eksikliği hosted cold-start outlier'ına katkı vermiş olabilir [S117]. Probe şimdi ilk source assignment'dan önce en çok 5 saniye `Loaded` bekler, window close'u lifetime cancellation ile zincire bağlar ve readiness timeout'unu ayrı sınıflandırır. Ölçülen startup aralığı, `≤3000 ms` p95/`≤5000 ms` maximum, HLS-first ve yalnız explicit `MediaFailed` için bounded retry kuralları değişmez. Hipotez henüz hosted PASS ile doğrulanmamıştır.
