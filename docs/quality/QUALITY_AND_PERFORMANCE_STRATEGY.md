...
Run number `#25` UI kaydı, `test(windows): spike protected catalog layout` başlıklı commit `786151cef6c60de5e34009ebb6643ec804490ee9` için 3/3 işi yeşil, üç artifact ve `8m32s` toplam süre gösterir. Commit-bound normal workflow adayı solution'ın Debug/Release x64 build'lerinde derlemiş; architecture guard'ları production graph/normal-workflow ayrımını denetlemiş ve package smoke çıkarılmış production MSIX'te `IptvSuite.ProtectedCatalogSpike*` denylist'ini uygulamıştır. Bu compile/isolation/package-exclusion zinciri workflow/UI düzeyinde **VERIFIED**dır. Normal workflow opt-in wrapper'ı çağırmadığından `Smoke` veya `Decision` çalıştırılmamış, candidate evidence yüklenmemiştir; exact hosted test sayısı ile artifact JSON/ZIP/içerik/tam digest bağları bağımsız doğrulanmamıştır. Heavy comparative `Decision` ve production seçimi açık, M4 `IN PROGRESS`, ADR-003 `Proposed` kalır.

Reddedilen per-record DPAPI bulk layout'ıyla karşılaştırılacak `IptvSuite.ProtectedCatalogSpike`, solution build'inde derlenen fakat normal quality/hosted akışta executable olarak çağrılmayan, nonpackable/nonpublishable x64 test-only adaydır; production project graph'ı ve MSIX dışındadır, package denylist'i assembly/artifact adlarını reddeder. Hafif kontrol komutu `.\eng\Invoke-WindowsProtectedCatalogSpike.ps1 -Mode Smoke`dur. Source/snapshot başına immutable AES-256-GCM container'ı big-endian/footerless'tır; activation öncesi structural reopen + CurrentUser DPAPI unwrap + en fazla 16 eşit aralıklı tag probe yapar. Adapter reopen aynı process'tedir, cold restart değildir; cancellation progress'i committed record değil encrypted staged record'dur. Exact SDK `10.0.302` dirty-worktree `Smoke`, Release x64 build'de 0 warning/error, 1.000 kayıt × 1 tur, 22 true reader-validation Boolean'ı, onuncu encrypted staging kaydından sonra sıfır post-cancel encryption/activation/post-completion mutation/temp artifact ve canary/cleanup PASS sonucu üretmiştir; targeted architecture suite `14/14` PASS'tir. `20469` byte summary SHA-256 `9d52172e61416fa1c73206ec745bd72a5aa18f15844153b08026c4597494cb99`; spec `2693eed7032d4c8ea50a062cd8b123ba782a89293e9d31b079335d3f6aa15666`, lock `fa138ce4203c6967edaeb8031722d72fd8372f1ed08bd9a52dff6342b582e730`, runner `a7697286871d17a96abe4c238975c57bd5bc1878c70735ead632d963c7989cf3` hash'lerine bağlı ve `decisionEligible=false`dır. Aynı worktree'de ayrıca güncel local full gate, architecture 14 + unit 151 + integration 34 = 199/199 testi iki aynı `Passed` setiyle ve fixture/sentinel/scanner/artifact-canary kontrolleriyle geçmiştir; summary `commitSha=null` taşıdığı için commit-bound değildir. Bu Smoke veya full gate performans kabulü değildir ve full gate aday executable'ını çalıştırmaz. Ağır 5k–50k × 20 `Decision` hiç çalıştırılmamıştır; fresh explicit onay, clean worktree ve `-AllowDecision` ister. Container production-ready veya tercih edilen tek SQLite transaction'lı key-state + ciphertext-row + active-snapshot tasarımı değildir. Ayrıntı: [M4 protected-catalog aday kaydı](M4_PROTECTED_CATALOG_CANDIDATE.md).
...
- Per-record DPAPI bulk-locator layout'ı bütçeyi karşılamadı. Test-only immutable-container adayının threat model/formatı ve dirty-worktree Smoke'u kayıtlıdır; fakat fresh-onaylı clean-worktree 5k–50k × 20 `Decision`, aynı protokolde comparative yorum ve tercih edilen SQLite transaction tasarımının production kanıtı açıktır.
