...
| M15 | `IN PROGRESS / BLOCKED, 2026-08-26`: clean `0236894f8cc8c6212305f5ba9c294e335f3d031e` schema-v1 static audit'te `technicalBaselinePassed=true`, `releaseReady=false`; exact 8 asset, 4 schema-v2 lockfile, 23 package adı ve tarihsel 16 blocker. Run #223 attempt 2 (`32881949188`) exact install-root runtime audit'ini sıfır mutation ve pre/post equivalent `true` ile geçti. Schema-v2 x64-only disposition `Arm64ReleaseDecisionPending` blocker'ını kapattı. İlk hosted SBOM/CVE kabulleri tarihsel olarak run #226 ve #4'tür. Güncel bağımsız SBOM producer run `#1` (`32918708589`) signed package-bound SPDX `2.2` zincirini official+strict validation ile yeniden geçti; buna ledger hash'iyle zincirlenen CVE producer run `#10` (`32919649968`) exact 4-project/4-lockfile/23-package graph'ını `4/0/4` audited restore, sıfır suppression/override ve `0/0/0` bilinen vulnerability ile yeniden geçti. Son pre-asset accepted schema-v4 ledger, exact contract/graph değişmediği ve validator-owned yedi günlük pencere fresh kaldığı sürece `SbomPending` ve `CveReviewPending` blocker'larını kapatmış; `releaseReady=false` ve 13 blocker bırakmıştır. WACK/private Store, production identity/lifecycle, license/NOTICE/privacy/codec-IP hukuk/signing ve diğer kapılar açık. [Baseline](M15_RELEASE_READINESS_BASELINE.md) |
...
`M15 SBOM/CVE acceptance renewal — HOSTED VERIFIED, 2026-08-26`: Bağımsız SBOM producer run `#1` (`32918708589`), clean `e7564aa9382fb12b10869866811e75847d1f6f16` üzerinde DWM refresh-gap normalizasyonu ve exact maximum korumasını içeren package smoke'u geçti; acceptance ledger/contract SHA-256 değerleri `9eaf890f027f3485e89851701cfbfe4676afb44be4a918f525290c58385d9a21` / `e5324fafa743cd003af480ab2c521ae2181a306fd67704c221d905025a7fccae`dir. Buna zincirlenen CVE producer run `#10` (`32919649968`), commit `15826e5ecad4a4b5737aa7bf39d28c62b0e71de7`, job `98030516890` ve artifact `9589408603` ile aynı exact graph'ı yeniden `0/0/0` bulguyla geçti. Güncel CVE ledger SHA-256 `01180e5c90816fda2f1712a2ecd5ed634399caee76d68b7ffeb47c178e2aa208`, freshness sonu `2026-09-02T01:38:42Z`dir. Threshold, blocker ve non-claim kapsamı değişmedi.

`M15 deterministic asset provenance — IMPLEMENTED / RE-ACCEPTANCE PENDING, 2026-08-26`: Exact sekiz production-package PNG/ICO asset'i source-controlled ve dış girdisiz deterministic tariften yeniden üretildi; strict ledger path, dimension/ICO-frame, length, hash ve canonical asset-set bağını doğrular. Asset'ler yalnız nötr development placeholder'dır; marka/logo onayı, copyright ownership, redistribution, root `LICENSE`/`NOTICE`, Store/legal veya codec-IP kabulü değildir. Byte değişimi eski package snapshot'ını değiştirdiği için yukarıdaki 13-blocker schema-v4 sonucu tarihsel accepted checkpoint olarak kalır; yeni hosted package SBOM/CVE zinciri tamamlanmadan schema-v5 için final 12-blocker/readiness sonucu kaydedilmez.
