...
| M15 | `IN PROGRESS / BLOCKED, 2026-08-26`: clean `0236894f8cc8c6212305f5ba9c294e335f3d031e` schema-v1 static audit'te `technicalBaselinePassed=true`, `releaseReady=false`; exact 8 asset, 4 schema-v2 lockfile, 23 package adı ve tarihsel 16 blocker. Run #223 attempt 2 (`32881949188`) exact install-root runtime audit'ini sıfır mutation ve pre/post equivalent `true` ile geçti. Schema-v2 x64-only disposition `Arm64ReleaseDecisionPending` blocker'ını kapattı. Run #226 (`32897767622`) signed package-bound SPDX `2.2` SBOM'u official+strict validation ile geçirdi; schema-v3 ledger yalnız `SbomPending` blocker'ını kapattı. Ayrı run #4 (`32912296486`), clean `2053f4099819b3bb19bb9dd3370d60f0161098f1` üzerinde exact 4-project/4-lockfile/23-package production graph'ını `4/0/4` audited restore, sıfır suppression/override ve `0/0/0` bilinen vulnerability ile geçti. Schema-v4 ledger, exact 16-file contract ve graph değişmediği ve validator-owned yedi günlük pencere fresh kaldığı sürece yalnız `CveReviewPending` blocker'ını kapatır; `releaseReady=false` ve 13 blocker kalır. WACK/private Store, production identity/lifecycle, license/NOTICE/privacy/codec-IP hukuk/signing ve diğer kapılar açık. [Baseline](M15_RELEASE_READINESS_BASELINE.md) |
