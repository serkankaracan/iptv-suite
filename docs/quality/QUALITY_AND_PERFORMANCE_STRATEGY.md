...
Schema-4 controller, açıkça istenen sayıda mapped loopback medya isteğine tek-response sentetik `503` enjekte eder; scheduled/injected/recovered sayaçlarının tam eşitliği olmadan evidence yayımlamaz. 2026-08-21 kısa Windows Client koşusu bir interruption ve bir sonraki başarılı mapped response'u 100/100 switch içinde doğruladı; [network kaydı](M10_NATIVE_TIER_A_NETWORK_RECOVERY_EVIDENCE.md) yalnız bu fault türünü **PARTIAL VERIFIED** yapar. Sekiz saat koşusunda saatlik schedule ayrıca doğrulanmalıdır.
...
| M10 | `CANDIDATE NO-GO; NATIVE FALLBACK IN PROGRESS, 2026-08-21`: exact LibVLC seti GPL-zero hard gate FAIL; ADR-002 rejected. ADR-007 native Tier A fallback gerçek Windows Client'ta 100 switch + 120 dakika developer soak PASS; resize/minimize/restore/fullscreen ve tek HTTP interruption/recovery alt kümeleri PARTIAL PASS. 8 saat, kalan surface/network ve device/HW-decode matrisi açık. [Candidate](M10_PLAYBACK_CANDIDATE_DECISION.md), [developer soak](M10_NATIVE_TIER_A_DEVELOPER_SOAK_EVIDENCE.md), [surface](M10_NATIVE_TIER_A_SURFACE_EVIDENCE.md), [network](M10_NATIVE_TIER_A_NETWORK_RECOVERY_EVIDENCE.md) |
