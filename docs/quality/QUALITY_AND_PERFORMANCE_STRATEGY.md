...
`M15 development identity WACK preflight — IMPLEMENTED / EXECUTION PENDING, 2026-08-26`: Manual `run_wack=true` yolu yalnız disposable development identity üzerinde exact `appcert reset` → bounded installed-package test sırasını çalıştırır. Ham WACK XML/stdout/stderr kalıcı veya upload artifact'ı değildir; strict parser yalnız PASS/non-partial sonucu kabul eder ve sanitized summary `ClosedBlocker=None`, `ReleaseReady=false` taşır. Bu preflight final RC-bound WACK/Partner Center kabulü değildir; `WackPending` açık kalır. Değişen package/workflow contract'ı için SBOM/CVE acceptance zinciri yeni clean checkpoint üzerinde yenilenmeden readiness sonucu tekrar kabul edilmez.
