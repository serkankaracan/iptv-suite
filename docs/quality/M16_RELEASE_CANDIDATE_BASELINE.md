...
`windows-quality.yml` içindeki `run_m16_final_artifacts` yalnız manual dispatch input'udur ve `run_wack` ile aynı koşuda seçilemez. Upload allowlist'i yalnız sanitized final JSON'dur; raw yüzeyler workflow artifact'ı değildir. [Hosted run `33045343924`](https://github.com/serkankaracan/iptv-suite/actions/runs/33045343924), clean commit `32554919db648f84b304f7380f645040c4f933d8` üzerinde bütün required job zincirini ve dört yüzeyli final taramayı geçti; producer job `98430451486`, Required Windows gate `98432129275` ile başarıyla tamamlandı. Artifact `9635895663` digest'i `feb50cc4545bb4a66f4f350468031f83951fb277a38fbe37d217046bd1e612d0`; allowlist'teki tek `last-success.json` üyesinin SHA-256 değeri `b01175e41f897607077d8e01c50cb2c70df904abb498c9ebb731e740d416b83d`dır. Exact `owned-app-data`, `exact-package`, `support-artifact` ve bounded `full-log` yüzeylerinin dördü de `clean`, toplam finding sayısı `0`, `SameBuildBindingPassed=true`, `RepositoryStable=true` ve `RawSurfacesUploaded=false` sonucunu verdi. Aynı-run package artifact'ı `9635894986`, package/SBOM bağını exact commit ve `02fad1f34e5be645ccd0b6587257ebb3bb2d36c623445be1d15985a16611474f` package SHA-256 değeriyle doğrular. Bu commit-bound current kabul yalnız `M16FinalArtifactCanaryScanPending` kapısını kapatır; feature freeze, 24 saat soak, fiziksel matrix, Store/WACK, hukuk veya release kararı üzerinde sonuç üretmez.

Source-controlled `eng/windows-m16-final-artifact-acceptance.json` kabul ledger'ının raw SHA-256 değeri `0df5c5524c75f7f26a64a93e38541deb1aec0019515cb1ae6f3bea503f72c535`dir. Producer contract bağı exact `39` source'u; text girdilerini strict UTF-8/no-BOM ve LF-normalized byte'larla, binary `.ts` fixture girdisini raw byte'larla `d92a3cd5ad8ac55d4c97906214eb401871ef52fd5d25259ff230e31a98f8982b` SHA-256 değerine bağlar. Current package-producing snapshot exact `115` dosya ve `5568fb8fc87f614392762501cb2a4b3be1a13487bb8cfab037ccaec579756810` SHA-256 bağıdır. Ledger raw byte/schema/artifact tutarsızlığı hard-invalid kalır ve RC evidence üretmez. Intact ledger current producer source veya package snapshot'ından geri kalırsa aggregator yine `result=stale-reopen`, `current=false` yayımlar ve `M16FinalArtifactCanaryScanPending` blocker'ını yeniden açarak blocked evidence üretmeye devam eder.
...
`HOSTED VERIFIED / CURRENT ACCEPTED, 2026-08-27`: [Run `33045321525`, attempt `2`](https://github.com/serkankaracan/iptv-suite/actions/runs/33045321525), clean `32554919db648f84b304f7380f645040c4f933d8` ve exact SDK `10.0.302` ile iki deterministik full quality runında `631/631` sonucu geçti. Quality job `98432568923` (`2026-08-27T06:54:11Z`) ve Required Windows gate `98436834409` başarıyla tamamlandı. Canonical full-result set SHA-256 değeri `66dab64fa75e52da441dd863490f8d0c5c32f54c5963a12b860ff8af19663ff2`dir. Bunun içindeki exact `77` ArchitectureTests sonucu birer kez ve `Passed` olarak bulundu; ordinal canonical altküme SHA-256 değeri `9d2e961e127593313f48365a9c7f700a6bf1e745c832c8947a94c90a0c4da778`dir. Quality sentinel fail/recovery, scanner CLI contaminated/clean self-test'i ve artifact-files-only canary scan da geçti. `windows-quality-evidence` artifact `9636240410`, `14.137` byte ve SHA-256 `00c53a61af8fd19bece91e484d5f6427f8401cb12dfece48bb5c77c7e0838a7c`dır; içindeki `47.236` byte `evidence/quality-summary.json` üyesinin SHA-256 değeri `19ee05ba79f27ac21a276174bd6cc852e1e32d7efed709cdde3a180905d7b367`dir.

Source-controlled `eng/windows-m16-security-architecture-acceptance.json` ledger'ının raw SHA-256 değeri `33ef854004876c307a7edbf73a90a42d28d8dc0e7cb7d332f980f79d0c0d3fbf`dır. Exact git-tracked workflow, build/toolchain, `eng`, production, test ve testdata closure'ı `329` source ve LF-normalized text/raw binary toplam `7.159.419` canonical byte'tır; ordinal path sıralı binding SHA-256 değeri `e532e65e7633fc0effc21e2765c5cd22926b938008aa6095f377abf134eeb465`dir. Acceptance/readiness ledger ve validator döngüleri exact exclusions ile closure dışında kalır; M15 predecessor zinciri ayrıca fail-closed doğrulanır. Run `33045321525` attempt `2` bu exact closure'ı hosted quality artifact'ına bağladığından aggregator security/architecture kaydını `accepted-current` sayar ve yalnız `M16FinalSecurityArchitectureScanPending` kapısını kapatır. Missing/tamper/schema/artifact bağı hard-invalid; sonraki closure drift'i yine `stale-reopen`dır.
...
- Son historical RC evidence'ındaki `m1ToM15AutomatedGateSetPassed=false`, o evaluation anındaki stale SBOM predecessor sonucunu kaydeder. Current source-controlled M15 zinciri hosted SBOM run `33042549429` ve CVE run `33043196165` ile `technicalBaselinePassed=true` durumuna yenilenmiştir; bu alan ancak current RC input seti yeniden aggregate edildiğinde değişebilir.
