...
`windows-quality.yml` içindeki `run_m16_final_artifacts` yalnız manual dispatch input'udur ve `run_wack` ile aynı koşuda seçilemez. Upload allowlist'i yalnız sanitized final JSON'dur; raw yüzeyler workflow artifact'ı değildir. [Hosted run `33127559624`, attempt `1`, run number `#321`](https://github.com/serkankaracan/iptv-suite/actions/runs/33127559624), clean commit `1e42603173e69e536027a0a094338da533faa97d` üzerinde bütün required job zincirini ve dört yüzeyli final taramayı geçti; producer job `98712013301`, Required Windows gate `98713628778` ile başarıyla tamamlandı. `windows-m16-final-artifact-evidence` artifact `9669457755`, `998` byte ve SHA-256 `bbc03adaaafc2d790e2c5f5c559c18c012c3f5f0f4aa0aa5b4f2c42a506289a6` digest'ini taşır; allowlist'teki tek `3.281` byte `last-success.json` üyesinin SHA-256 değeri `8e7654776bf532e76e555056159079d5e40506fdf2c03a916d59e7f09976d91b`dir. Exact `owned-app-data`, `exact-package`, `support-artifact` ve bounded `full-log` yüzeylerinin dördü de `clean`, toplam finding sayısı `0`, `SameBuildBindingPassed=true`, `RepositoryStable=true` ve `RawSurfacesUploaded=false` sonucunu verdi. Aynı-run `windows-msix-smoke-evidence` artifact'ı `9669456800`, `7.846` byte ve SHA-256 `776dafdcc2cdf9696604f3e59af746dc4cd32acb0e22110fef67762472030955` digest'iyle package/SBOM bağını exact commit ve `3e31ee5b010a07f8912ba5c4bd1cf094d20d182f85301abbd1414cc5b8326a2b` package SHA-256 değerine bağlar. Bu commit-bound current kabul yalnız `M16FinalArtifactCanaryScanPending` kapısını kapatır; feature freeze, 24 saat soak, fiziksel matrix, Store/WACK, hukuk veya release kararı üzerinde sonuç üretmez.

Source-controlled `eng/windows-m16-final-artifact-acceptance.json` kabul ledger'ının raw SHA-256 değeri `89f87e92514cda2cbfc8d41f34a6d074d69871a6808086fb15876e0d0d95c05f`dır. Producer contract bağı exact `39` source'u; text girdilerini strict UTF-8/no-BOM ve LF-normalized byte'larla, binary `.ts` fixture girdisini raw byte'larla `f033e9721efd06184d5bc21c4fd215902b3c1e98fa3d5279cbd644e0d171f6bc` SHA-256 değerine bağlar. Current package-producing snapshot exact `115` dosya ve `5568fb8fc87f614392762501cb2a4b3be1a13487bb8cfab037ccaec579756810` SHA-256 bağıdır. Ledger raw byte/schema/artifact tutarsızlığı hard-invalid kalır ve RC evidence üretmez. Intact ledger current producer source veya package snapshot'ından geri kalırsa aggregator yine `result=stale-reopen`, `current=false` yayımlar ve `M16FinalArtifactCanaryScanPending` blocker'ını yeniden açarak blocked evidence üretmeye devam eder.
...
`HOSTED VERIFIED / CURRENT ACCEPTED, 2026-08-28`: [Run `33127559624`, attempt `1`, run number `#321`](https://github.com/serkankaracan/iptv-suite/actions/runs/33127559624), clean `1e42603173e69e536027a0a094338da533faa97d` ve exact SDK `10.0.302` ile iki deterministik full quality runında `631/631` sonucu geçti. Quality job `98709207462` (`2026-08-28T00:03:21Z`) ve Required Windows gate `98713628778` başarıyla tamamlandı. Canonical full-result set SHA-256 değeri `66dab64fa75e52da441dd863490f8d0c5c32f54c5963a12b860ff8af19663ff2`dir. Bunun içindeki exact `77` ArchitectureTests sonucu birer kez ve `Passed` olarak bulundu; ordinal canonical altküme SHA-256 değeri `9d2e961e127593313f48365a9c7f700a6bf1e745c832c8947a94c90a0c4da778`dir. Quality sentinel fail/recovery, scanner CLI contaminated/clean self-test'i ve artifact-files-only canary scan da geçti. `windows-quality-evidence` artifact `9669277637`, `14.133` byte ve SHA-256 `bea5dd2a0c87e70bfa26065b66b7fba9c7ffc2acb8193bad22d2e5d5fecfdd86`dır; içindeki `47.236` byte `evidence/quality-summary.json` üyesinin SHA-256 değeri `9d865417ff927ef8ddf532fcd9706f657d0200cb6251e3f21b7ba06c08b4528e`dir.

Source-controlled `eng/windows-m16-security-architecture-acceptance.json` ledger'ının raw SHA-256 değeri `fca033be76fb252ed2a4f3ac9dbf36dbcf0240b1f4eeb1dfd06210d79b7ca152`dir. Current producer contract exact `329` source / `7.204.158` canonical byte ve `7ee590883be2198547a336611370a629c5f20a7f16a5cdd6e3b9294fb626c031` SHA-256 bağındadır. Run `#321` bu final closure'ı doğruladığı için ledger `accepted-current` olur ve yalnız `M16FinalSecurityArchitectureScanPending` kapısını kapatır. Acceptance/readiness ledger ve validator döngüleri closure dışında; source drift'i kapıyı `stale-reopen` ile yeniden açar, missing/tamper/schema/artifact bağı ise hard-invalid kalır.
...
Tam RC aggregation baseline'ı gerçek current predecessor input setiyle henüz yeniden üretilmemiştir. Hosted final-artifact, journey ve security/architecture kayıtlarının üçü de kendi exact commit/artifact ve source closure bağlarında current accepted evidence'dır. Ortak run `33127559624`, journey için exact `132`-source closure'ı; final-artifact ve security/architecture için ise final `39`-source artifact ile `329`-source security closure'larını locked quality, signed MSIX lifecycle, DPAPI ve required gate ile tamamen yeşil doğruladı. Bu kayıtlar kalan predecessor veya dört M16 kapısının tamamlandığı iddiası değildir.
