...
`windows-quality.yml` içindeki `run_m16_final_artifacts` yalnız manual dispatch input'udur ve `run_wack` ile aynı koşuda seçilemez. Upload allowlist'i yalnız sanitized final JSON'dur; raw yüzeyler workflow artifact'ı değildir. [Hosted run `33145434224`, attempt `1`, run number `#327`](https://github.com/serkankaracan/iptv-suite/actions/runs/33145434224), clean commit `c5db51438b3fd9247c92fa2946b13ad09147f9cc` üzerinde bütün required job zincirini ve dört yüzeyli final taramayı `2026-08-28T06:00:37Z` zamanında geçti; producer job `98767157714`, Required Windows gate `98768606459` ile başarıyla tamamlandı. `windows-m16-final-artifact-evidence` artifact `9675962291`, `999` byte ve SHA-256 `f7b3c777ba1941642e173be82490ee6478d470e9ab189f8b44c3ca7e459451fb` digest'ini taşır; allowlist'teki tek `3.281` byte `last-success.json` üyesinin SHA-256 değeri `383a69a75eb9f02547dc8c0aa0a382e05cbc626138ac11be2b2e70f8389b0107`dir. Exact `owned-app-data`, `exact-package`, `support-artifact` ve bounded `full-log` yüzeylerinin dördü de `clean`, toplam finding sayısı `0`, `SameBuildBindingPassed=true`, `RepositoryStable=true` ve `RawSurfacesUploaded=false` sonucunu verdi. Aynı-run `windows-msix-smoke-evidence` artifact'ı `9675961825`, `7.779` byte ve SHA-256 `2469905d082bcd6e7021ea622f2b68a704c6b13311e187b5b82e0b0f4f8769c1` digest'iyle package/SBOM bağını exact commit ve `a809f0d41c188824634d9af4c75bde15f3a378190845ed93a58e7b49aff17e97` package SHA-256 değerine bağlar; iç package evidence, SBOM summary ve SPDX üyelerinin SHA-256 değerleri sırasıyla `f09391799c2056d0bb57a127b97b8a5d93bac6c7bd47b98405e45c0d3fe6dd0d`, `1af487ffe76e5f34e925f88d962254598a58f38abf9dd27f26400c79120ee568` ve `f2c0faafd7b46a464f7ad1b035aaecaa1db75076fb88af8c3dadf35462ac698a`dır. Bu commit-bound current kabul yalnız `M16FinalArtifactCanaryScanPending` kapısını kapatır; feature freeze, 24 saat soak, fiziksel matrix, Store/WACK, hukuk veya release kararı üzerinde sonuç üretmez.

Source-controlled `eng/windows-m16-final-artifact-acceptance.json` kabul ledger'ının raw SHA-256 değeri `327a09c059111e62682d247c06f07249ede8c4d1cc319f2b7b85efff48ca6364`dır. Producer contract bağı exact `39` source'u; text girdilerini strict UTF-8/no-BOM ve LF-normalized byte'larla, binary `.ts` fixture girdisini raw byte'larla `697856156c64898518dd30cd3013671939206d4f59bb15414973fbd01be2f63e` SHA-256 değerine bağlar. Current package-producing snapshot exact `115` dosya ve `5568fb8fc87f614392762501cb2a4b3be1a13487bb8cfab037ccaec579756810` SHA-256 bağıdır. Ledger raw byte/schema/artifact tutarsızlığı hard-invalid kalır ve RC evidence üretmez. Intact ledger current producer source veya package snapshot'ından geri kalırsa aggregator yine `result=stale-reopen`, `current=false` yayımlar ve `M16FinalArtifactCanaryScanPending` blocker'ını yeniden açarak blocked evidence üretmeye devam eder.

## Bounded sentetik uçtan uca entegrasyon journey'si

`HOSTED VERIFIED / CURRENT ACCEPTED, 2026-08-28`: `M16SyntheticEndToEndJourneyTests`, tamamen sentetik ve TLS certificate-pin'li loopback veriyle production onboarding, bounded transport/probe, protected-reference import, SQLite browse/search, playback coordinator control, watchdog fault/reconnect ve source deletion/tombstone zincirini tek bounded testte yürütür. Exact HTTP istek sırası, ikinci fiziksel playback session'ına control restore sırası, stop-before-reopen, retired-source non-admission, disposed lease'ler, protected-record zeroization ve ana SQLite dosyasıyla mevcut `-journal`/`-wal`/`-shm` yan yüzeylerinde raw locator yokluğu doğrulanır. Test timeout'u sabit `60 s`dir.

[Hosted run `33127559624`, attempt `1`](https://github.com/serkankaracan/iptv-suite/actions/runs/33127559624) clean `1e42603173e69e536027a0a094338da533faa97d` üzerinde exact journey sonucunu iki deterministik run'da geçti. Quality job `98709207462`, Required Windows gate `98713628778` ve workflow sonucu `success` oldu. `windows-quality-evidence` artifact `9669277637`, `14.133` byte ve SHA-256 `bea5dd2a0c87e70bfa26065b66b7fba9c7ffc2acb8193bad22d2e5d5fecfdd86` digest'ini taşır; `evidence/quality-summary.json` üyesi `47.236` byte ve SHA-256 `9d865417ff927ef8ddf532fcd9706f657d0200cb6251e3f21b7ba06c08b4528e`dır. Summary exact `631` test/run, `cleanRunCount=2` ve tek `AuthorizedRemotePlaylistCompletesExactSyntheticReleaseCandidateJourney|Passed` sonucu taşır.

Bu current kabul `eng/windows-m16-synthetic-journey-acceptance.json` ledger'ında raw SHA-256 `0d209c9a5988c4b775e2ce626e9035503870bd51ccc9920102d5e42101db30e0` ile kayıtlıdır. Exact `132` source'un producer/dependency closure SHA-256 değeri `033c336b18ab991f43edd67d4fb346ac74bab9c78bdd24c6bb8467fc4786fae8`dir. Ledger current kaldığı sürece yalnız `M16SyntheticEndToEndJourneyPending` kapısını kapatır; closure drift'i `stale-reopen` ile kapıyı yeniden açar. Raw ledger/schema/artifact tutarsızlığı ise stale değil hard-invalid'dir.
...
`HOSTED VERIFIED / CURRENT ACCEPTED, 2026-08-28`: [Run `33145434224`, attempt `1`, run number `#327`](https://github.com/serkankaracan/iptv-suite/actions/runs/33145434224), clean `c5db51438b3fd9247c92fa2946b13ad09147f9cc` ve exact SDK `10.0.302` ile iki deterministik full quality runında `634/634` sonucu geçti. Quality job `98765280906` (`2026-08-28T05:51:23Z`) ve Required Windows gate `98768606459` başarıyla tamamlandı. Canonical full-result set SHA-256 değeri `a94e0c17dfa7ab56b098a760101c4b2afe10bc2b0b2aa542d0b790da049c23b4`dir. Bunun içindeki exact `80` ArchitectureTests sonucu birer kez ve `Passed` olarak bulundu; ordinal canonical altküme SHA-256 değeri `8bf658b6c8e724bf1af1f0c48f67b9a6c6b65587e137aba6ebcfbc4e07f27bfb`dir. Quality sentinel fail/recovery, scanner CLI contaminated/clean self-test'i ve artifact-files-only canary scan da geçti. `windows-quality-evidence` artifact `9675779615`, `14.190` byte ve SHA-256 `75f74ef497cff27fbbd725be81a61104e0bd16e59370fb6520f379a94002a772`dır; içindeki `47.549` byte `evidence/quality-summary.json` üyesinin SHA-256 değeri `1ca66e4a9ff89e8be3fc9b95adfdff9f9d3d0a9ad0b11f4e8683d927122df2f7`dir.

Source-controlled `eng/windows-m16-security-architecture-acceptance.json` ledger'ının raw SHA-256 değeri `1c8cb5fb9fa579d4c91cdbc1427044dc2de563f25cf30d449210e8f703652d22`dir. Current producer contract exact `330` source / `7.208.153` canonical byte ve `c9ebdfcd3d0544c8e4209ba4d6cf3c9f1dbfe2919162ee603e17cc13a0d6ec1d` SHA-256 bağındadır. Run `#327` bu final closure'ı doğruladığı için ledger `accepted-current` olur ve yalnız `M16FinalSecurityArchitectureScanPending` kapısını kapatır. Acceptance/readiness ledger ve validator döngüleri closure dışında; source drift'i kapıyı `stale-reopen` ile yeniden açar, missing/tamper/schema/artifact bağı ise hard-invalid kalır.
...
Current final-artifact, security/architecture ve journey ledger'ları kendi automated kapılarını kapatır. Blocked baseline aşağıdaki exact dört M16 kapısını açık tutar:
