...
**Durum:** yalnız test amaçlı immutable-container comparative spike `GO`; bütün production yerleşimleri `NO-GO`; M4 `IN PROGRESS`; ADR-003 `Proposed`

## 1. Sonuç

50.000 channel locator için file-per-record DPAPI düzeni önceki `Decision` koşusunda reddedilmiştir. Bu kayıt, onun yerine production implementation seçmez.

| Seçenek | M4 kararı | Gerekçe |
|---|---|---|
| DPAPI-wrapped source/snapshot DEK + record-başına AES-256-GCM kullanan **test-only immutable container** | Comparative spike için `GO`; production için `NO-GO` | Standart .NET kripto primitive'leriyle row-başına `Protect` yerine staging attempt başına tek `Protect` kullanan en küçük karşılaştırma yüzeyidir. Pre-activation doğrulama ve sonraki adapter açılışı ayrı `Unprotect` çağrıları yapar. Dış katalog metadata'sı ve active pointer ile atomik commit sağlamaz. |
| Aynı key-state ve ciphertext row'larını tek SQLite veritabanında yöneten transactional state machine | Gelecek için tercih edilen production adayı; bugün `NO-GO` | Tek transaction old-or-new activation ve recovery için doğru yöndür; fakat exact schema/provider/dependency, crash matrix, migration, delete ve 50k ölçümü seçilmemiştir. Mevcut container prototipi bu veritabanı değildir. |
...
**Net karar:** encrypted database M4 production kapsamına alınmaz. SQLCipher Commercial yalnız procurement ve dependency review sonrasında yeniden açılabilecek bir adaydır. Şimdi yalnız immutable-container spike'ı hazırlanabilir; ağır `Decision` koşusu fresh açık onay olmadan çalıştırılamaz.
...
4. Crash/fault, packaged lifecycle, gerçek wrong-user, migration ve source-wide reconciliation kanıtı.
5. Aynı comparative protokolde clean-commit `Decision`, sonra gerçek parser/normalize/index end-to-end referans-cihaz koşusu.
