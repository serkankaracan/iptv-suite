...
Run #148, `CompositionTarget.Rendered` davranışının bu dar startup hipotezi için çözüm olmadığını doğruladığından bu davranış takip checkpoint'inden çıkarılır. Bir sonraki salt diagnostic, `MediaSource.OpenOperationCompleted` sonucunu ve zamanını active failure snapshot'ına bağlayarak source-open tamamlanması ile sonraki `MediaOpened` bekleyişini ayırır [S120]. Handler attempt'e aittir, Source atamasından önce bağlanır ve reset/source disposal öncesinde `finally` içinde ayrılır. Bu event tek başına playback readiness veya başarı garantisi değildir; yeni retry, warm-up/sample exclusion ya da alternatif güvenlik yolu açmaz.

`≤3000 ms` p95, `≤5000 ms` maximum, source creation öncesindeki ölçüm başlangıcı, HLS-first sıra, tek player, yalnız explicit ilk `MediaFailed` retry kuralı, TLS/loopback sınırı ve fail-closed evidence/cleanup sözleşmeleri aynen korunur. `MediaSource.OpenOperationCompleted` attribution'ı successor hosted run ile değerlendirilmeden kök neden kapanmış sayılmaz. Hosted gate yeşil olmadan schema-10 cancellation/recovery başlamaz. Sekiz saatlik acceptance soak kullanıcı tarafından açıkça başlatılmadı ve bu checkpoint kapsamında başlatılmaz.
