...
`≤3000 ms` p95, `≤5000 ms` maximum, source creation öncesindeki ölçüm başlangıcı, HLS-first sıra, tek player, yalnız explicit ilk `MediaFailed` retry kuralı, TLS/loopback sınırı ve fail-closed evidence/cleanup sözleşmeleri aynen korunur. Run #149 source-open sonrası `MediaOpened` bekleyişini ilk kez ayırdı; attempt/deadline/cleanup bağını kapatan successor hosted run değerlendirilmeden kök neden kapanmış sayılmaz. Hosted gate yeşil olmadan schema-10 cancellation/recovery başlamaz. Sekiz saatlik acceptance soak kullanıcı tarafından açıkça başlatılmadı ve bu checkpoint kapsamında başlatılmaz.
