...
`M11 XTREAM CONSTRUCTION — VERIFIED, 2026-08-24`: Resolver, live provider-item playback'i source `ConfigurationId` owner'ı + exact `SecretReference` + persisted `SafeEndpoint` + canonical `ProviderItemKey` + `Hls|MpegTs` container tuple'ına bağlar. Protected payload operation-local açılır; credential lease her sonuçta kapanır, locator byte buffer'ı başarısızlıkta sıfırlanır ve yalnız başarıda kısa ömürlü `SecretLease`e devredilir. `/live/{username}/{password}/{providerItem}.{m3u8|ts}` route'u formal Xtream standardı olarak kabul edilmez; dar live-only `UNVERIFIED` uyumluluk varsayımıdır ve farklı biçimler fail-closed kalır.

`M11 PRODUCTION-PACKAGE ACCEPTANCE — VERIFIED, 2026-08-24`: Non-production x64 harness production Remote-M3U parser/import, catalog encryption ve DPAPI `CurrentUser` yoluyla yalnız sentetik bir Tier A kanal hazırlar; localhost HTTPS fixture'ı private key'i diske yazmadan GET/HEAD/single-range sunar ve IPC ticket'larında yalnız bounded aggregate sayaçlar taşır. Package controller public certificate'i exact thumbprint ile geçici `LocalMachine\\Root` trust'a alır, ikinci production activation'da kanalı UI Automation ile açıp `play → pause → resume → stop` safe-state zincirini doğrular, sonra exact process/certificate/package cleanup uygular. Harness ve testing assembly'leri production MSIX payload'ında yasaktır; önceki 50k UIA kabulü ve eşikler değişmemiştir. Local exact-SDK build architecture `35/35`, unit `171/171`, integration `116/116`; commit-bound run `32754184474` quality `354/354 × 2`, signed package playback/lifecycle, DPAPI ve required gate işlerini geçmiştir.

`M11 COMPLETION CHECKPOINT — 2026-08-24`: Application katmanı yalnız `SourceId + ChannelId` alan engine-neutral playback contract'ını ve app-owned single-session coordinator'ı taşır. Monoton session ID ile intent generation ayrıdır; engine mutasyonları tek gate üzerinden serialize edilir ve her session'ın lifetime token'ı yeni selection/stop/dispose ile iptal edilir. Rapid A→B önce fiziksel eski engine session'ını durdurur; stale veya illegal callback yeni state'i değiştiremez. `0..100` volume, mute ve `Fit`/`Fill` kontrolleri session-correlated ve nonterminaldir. Track inventory en çok `64` app-owned opaque ordinal taşır; unsupported seçim engine'e ulaşmadan reddedilir. Resolver remote-M3U ve Xtream live kayıtlarını exact active binding'lerden JIT çözer; secret/native kaynak sahipliği Infrastructure içinde kalır. Windows-native callback'ler exact context/generation/session/source bağında bastırılır; failure/stop/dispose source detach ve teardown zincirini kapatır. Packaged UI yalnız coordinator ve opaque selection görür, native controls'ü bypass etmez ve window-close'u playback/surface teardown sonrasına erteler. Raw locator, `Uri`, `SecretLease`, track label/native ID, Windows Media/WinUI ve spike tipleri public Application/catalog playback yüzeyine girmez. Hosted production-package acceptance ve exact evidence bağı tamamlanmıştır.

`M12 AUTOMATED CHECKPOINT — PARTIAL VERIFIED, 2026-08-25`: Power lifecycle policy, platformun `SystemSuspendStatus.Entering` sınırında current session'ı stop/drain eder ve resume'da kullanıcı intent'i olmadan yeniden başlatmaz [S131]. Exact source-release operation'ı yalnız matching session'ı bırakır; replacement session yarışında yeni session'ı durdurmaz. Run `32764149667` controls/fullscreen/focus/rapid-switch/resize/minimize/state/active-close otomasyonunu; run `32785306860` numeric short-run resource guard'ını; run `32792088083` source-delete success yolunu; run `32796910469` ise deterministic pending failure, restart admission block, exact katalog/configuration/tombstone/sibling korunumu, fault release, manual retry ve final katalog/DPAPI cleanup zincirini geçti. Windows-native player varsayılan system audio endpoint'ini kullanır [S132], fakat gerçek endpoint switch'i, fiziksel DPI/multi-monitor, sleep/wake ve Narrator `NOT RUN` kalır. [Kanıt](../quality/M12_COMPLETION_EVIDENCE.md).
