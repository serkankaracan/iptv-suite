...
`M11 XTREAM CONSTRUCTION UPDATE — 2026-08-24`: Resolver, live provider-item playback'i source `ConfigurationId` owner'ı + exact `SecretReference` + persisted `SafeEndpoint` + canonical `ProviderItemKey` + `Hls|MpegTs` container tuple'ına bağlar. Protected payload operation-local açılır; credential lease her sonuçta kapanır, locator byte buffer'ı başarısızlıkta sıfırlanır ve yalnız başarıda kısa ömürlü `SecretLease`e devredilir. `/live/{username}/{password}/{providerItem}.{m3u8|ts}` route'u formal Xtream standardı olarak kabul edilmez; dar live-only `UNVERIFIED` uyumluluk varsayımıdır ve farklı biçimler fail-closed kalır. Production-package playback kabulü açık olduğu için bu güncelleme M11 completion değildir.

`M11 PRODUCTION-PACKAGE ACCEPTANCE UPDATE — 2026-08-24`: Non-production x64 harness production Remote-M3U parser/import, catalog encryption ve DPAPI `CurrentUser` yoluyla yalnız sentetik bir Tier A kanal hazırlar; localhost HTTPS fixture'ı private key'i diske yazmadan GET/HEAD/single-range sunar ve IPC ticket'larında yalnız bounded aggregate sayaçlar taşır. Package controller public certificate'i exact thumbprint ile geçici `LocalMachine\\Root` trust'a alır, ikinci production activation'da kanalı UI Automation ile açıp `play → pause → resume → stop` safe-state zincirini doğrular, sonra exact process/certificate/package cleanup uygular. Harness ve testing assembly'leri production MSIX payload'ında yasaktır; önceki 50k UIA kabulü ve eşikler değişmemiştir. Local exact-SDK build, architecture `35/35`, unit `171/171` ve integration `116/116` geçmiştir; yönetici/hosted gerçek MSIX yürütmesi henüz `UNVERIFIED`dır.

`CURRENT M11 CHECKPOINT — 2026-08-24`: Application katmanı yalnız `SourceId + ChannelId` alan engine-neutral playback contract'ını ve app-owned single-session coordinator'ı taşır. Monoton session ID ile intent generation ayrıdır; engine mutasyonları tek gate üzerinden serialize edilir ve her session'ın lifetime token'ı yeni selection/stop/dispose ile iptal edilir. Rapid A→B önce fiziksel eski engine session'ını durdurur; stale veya illegal callback yeni state'i değiştiremez. `0..100` volume, mute ve `Fit`/`Fill` kontrolleri session-correlated ve nonterminaldir; coordinator başarılı open sonrasında desired değerleri play öncesinde exact session'a yeniden uygular. Track inventory en çok `64` app-owned, session-bound opaque ordinal taşır, listede olmayan veya selectable/capability ilanı bulunmayan track engine'e ulaşmadan reddedilir. Tier A adapter tek fixed audio ve sıfır subtitle bildirerek capability'yi dürüstçe kapatabilir; interface varlığı Tier B desteği değildir. Adapter exception/native metni yalnız stable `DomainError` koduna düşer. Internal `SqlitePlaybackSourceResolver`, remote-M3U kanalını exact active binding'den; Xtream live provider-item kanalını exact configuration owner/reference/endpoint/provider/container tuple'ından JIT çözer. Secret/locator lease'leri kısa ömürlüdür; strict UTF-8/HTTPS ve fail-closed binding kontrolünden geçmeyen değerler dispose edilip buffer'lar sıfırlanır. Internal `WindowsNativePlaybackEngine` lease'i adapter-private `MediaSource` oluşturulana kadar tutar; `MediaPlayer`/surface işlemlerini captured UI dispatcher'da serialize eder. Source-specific handler'lar assignment öncesi bağlanır; queued callback exact context+generation+session+source eşleşmeden state üretemez. Failure/stop context'i retire eder, handler'ları söker, source'u player'dan ayırıp tek kez dispose eder; final disposal surface'i ayırdıktan sonra player'ı kapatır. `MainPage` yalnız coordinator ve opaque selection görür; `ItemClick` kanal başlatır, fixed safe state UI dispatcher'da current session/state ile doğrulanır ve built-in transport controls kapalıdır. `MainWindow` ilk close'u iptal eder, page event/cancellation ömrünü kapatır, native playback'i surface yaşarken async dispose eder ve sonra normal close'a izin verir. Raw locator, `Uri`, `SecretLease`, track label/native ID, Windows Media/WinUI ve spike tipleri public Application/catalog playback yüzeyine girmez. Local package-acceptance controller/harness uygulanmış olsa da gerçek yönetici/hosted MSIX koşusu açık olduğundan bu checkpoint tek başına M11 completion kanıtı değildir.
