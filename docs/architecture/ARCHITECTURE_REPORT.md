...
`M12 AUTOMATED CHECKPOINT — PARTIAL VERIFIED, 2026-08-24`: Power lifecycle policy, platformun `SystemSuspendStatus.Entering` sınırında current session'ı stop/drain eder ve resume'da kullanıcı intent'i olmadan yeniden başlatmaz [S131]. Exact source-release operation'ı yalnız matching session'ı bırakır; replacement session yarışında yeni session'ı durdurmaz. Run `32764149667`, commit `8d537010f64193f7bc9baf05bc3707fb31d5362f` üzerinde controls, fullscreen/focus, rapid switch, resize, minimize/restore state ve active-close otomasyonunu geçti. Tek baseline/final resource snapshot'ı kalibrasyondur; numeric budget kabulü değildir. Windows-native player varsayılan system audio endpoint'ini kullanır [S132], fakat gerçek endpoint switch'i; fiziksel DPI/multi-monitor, sleep/wake, Narrator ve product source-delete entry point E2E ile birlikte `NOT RUN` kalır. [Kanıt](../quality/M12_COMPLETION_EVIDENCE.md).
