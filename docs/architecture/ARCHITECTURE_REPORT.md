...
**Belge durumu:** Windows yönü kararlaştırıldı; playback, secure-storage alternatif layout ve Samsung kararları spike koşullu
...
- Commit `fc96a211171d1e4f5e5f02174da6c565ef2d59bb` üzerindeki 5k/10k/20k/50k × 20 local `Decision` evidence'ında 50k create p95 `151.447 s` ve create allocation p95 `668.007 MiB`dir. `INFERENCE`: per-record DPAPI bulk channel-locator layout'ı tek başına 50k normalize + protected persistence + index için birleşik ≤3 s süre bütçesini ve bütün-import ≤150 MiB allocation bütçesini karşılamadığı için reddedilmiştir; bu source-credential kararı değildir. Reviewed alternatif güvenlik/layout kararı ve aynı protokolde comparative benchmark gerekir; aynı yerleşimi yeniden koşmak gate değildir. Ayrıntı: [M4 secret-store Decision evidence](../quality/M4_SECRET_STORE_DECISION_EVIDENCE.md).
- Ayrı `IptvSuite.ProtectedCatalogSpike`, source/snapshot başına immutable container'da fresh DEK'i bir kez CurrentUser DPAPI ile sarıp kayıtları AES-256-GCM ile koruyan test-only karşılaştırma adayıdır. X64 proje nonpackable/nonpublishable'dır; solution build'inde derlenir fakat executable'ı normal quality/hosted workflow tarafından çalıştırılmaz, production graph'ına veya MSIX'e girmez ve package denylist'i aday assembly/artifact adlarını reddeder. V1 format big-endian/footerless; pre-activation kontrol structural reopen + DPAPI unwrap + en fazla 16 eşit aralıklı tag probe'dur. Adapter reopen aynı process'tedir, cold restart değildir; staging ilerlemesi commit değil encrypted staged-record sayar. Exact SDK `10.0.302` dirty-worktree Smoke'u Release build'de 0 warning/error, 1k × 1, 22 true reader-validation Boolean'ı, 10 kayıt sonrası cancellation için sıfır post-cancel encryption/activation/post-completion mutation/temp artifact ve canary/cleanup PASS üretmiş; targeted architecture suite `14/14` geçmiştir. `20469` byte evidence SHA-256 `9d52172e61416fa1c73206ec745bd72a5aa18f15844153b08026c4597494cb99` ve `decisionEligible=false` taşır. Ağır 5k–50k × 20 `Decision` çalıştırılmamıştır; fresh explicit onay + `-AllowDecision` ister. Container production tasarımı veya key state + ciphertext rows + active pointer'ı tek SQLite transaction'la atomik değiştiren tercih edilen yön değildir; M4 `IN PROGRESS`, ADR-003 `Proposed` kalır. Ayrıntı: [M4 protected-catalog aday kaydı](../quality/M4_PROTECTED_CATALOG_CANDIDATE.md).
...
- provider contract;
- reviewed-alternative 5k–50k performance;
...
| O2 | INFERENCE / OPEN, 2026-08-14 | Per-record DPAPI bulk channel-locator layout'ı reddedildi; hangi reviewed alternatif güvenlik ve bütçe sınırlarını karşılar? | [Decision evidence](../quality/M4_SECRET_STORE_DECISION_EVIDENCE.md) + alternatif threat-model/design review + aynı protokolde comparative benchmark/fault injection | Security/Data / M4–M8 |
