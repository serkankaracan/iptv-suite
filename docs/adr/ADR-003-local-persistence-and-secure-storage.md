...
Pre-release v1 protected-store kayıtları bilerek korunur, fakat v2 store tarafından okunmaz veya silinmez. Released kullanıcı verisi ve güvenilir owner inventory'si olmadığından otomatik migration yapılmaz; bu, genel bir migration stratejisi kanıtı değildir. Hosted run `31735655363`, commit `6ac235f49ffd297196189e5e9441ad0c9e04f116` için architecture 12, unit 114 ve integration 27 olmak üzere 153/153 testi iki deterministik koşuda; signed package initialization smoke'unu ve coordinator'ı geçmiştir. Artifact'taki `ProtectedStoreDirectoryInitialized=true` yalnız o commit'in `LocalCache\ProtectedStore\v1` dizinini kanıtlar, owner-bound v2'yi kanıtlamaz. Decoder snapshot'ı 159/159 ve owner-v2 snapshot'ı 171/171 testi geçmiştir. Exact configuration-record deletion primitive'i dahil güncel worktree exact SDK ile Debug/Release x64 build; architecture 13, unit 137 ve integration 33 olmak üzere 183/183 testi iki aynı setle; fixture determinism, sentinel, scanner ve artifact-canary kontrollerini geçmiştir. 2026-08-14 GitHub Actions UI kaydı, run number `#15`in commit `6da2fc92a223c23d07f7a902de7607e16b1cbb8a` için 3/3 işi ve üç artifact'ı başarıyla tamamladığını gösterir; owner-v2 quality/package/lifecycle geçişi workflow/UI düzeyinde **VERIFIED**dir. Database run ID ile indirilen artifact JSON alan/hash bağı doğrulanmamıştır. Arbitrary mid-I/O cancellation/interleaving ve cross-process sıralama kanıtlanmış değildir.

M4'ün ayrı nonpublishable package-lifecycle hostu, production application graph'ına test mode eklemeden aynı disposable test identity/version ve CurrentUser altında üç ardışık process activation uygular: create, duplicate-create rejection ve verify-delete. DPAPI-protected control ticket source/configuration-owner/reference bağını activation'lar arasında taşır; create sonrası exact `ProtectedStore` ve lifecycle run-directory canary scan'i, final activation'da correct read, cross-owner negative, update/read/delete ve post-delete unavailable davranışı ile exact uninstall/app-data/certificate/output cleanup'ı hedefler. Package root'taki Windows-managed mutable registry hive'ları owned-write-surface taramasına dahil değildir. Harness package/output'u artifact değildir; yalnız allowlist'li metadata, Boolean sonuçlar ve hashlerden oluşan sanitized evidence upload edilebilir. Bu bir signed package update, reset/repair, uninstall-reinstall retention, production PFN, second-user veya crash-recovery testi değildir. Kod/workflow tanımı PASS sayılmaz; commit `6da2fc92a223c23d07f7a902de7607e16b1cbb8a` için run number `#15`in hosted zero-exit sonucu workflow/UI düzeyinde **VERIFIED**dir, artifact içeriği bağımsız doğrulanmış sayılmaz.

Bu ara sonuç ADR'yi `Accepted` yapmaz. Source-draft operation'ı yalnız doğrudan await edilen çağrıda store create sonrası reference'ı başarılı sonuçla döndürür. Üç ardışık activation kullanan same-version packaged harness'in hosted geçişi tamamlanmıştır; ancak gerçek update/reset/uninstall-reinstall lifecycle, wrong-user ve 5k–50k karşılaştırmalı ölçüme ek olarak durable authoritative owner metadata'sı, owner + reference + endpoint tuple swap ve M5 resolved-origin politikası, source-wide delete/startup protected-record orphan reconciliation, deletion/in-flight coordination ve check-to-use yarışını kapatan handle-relative path/reparse kararı henüz yoktur.
