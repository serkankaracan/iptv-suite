...
Immutable container yalnız crypto + sequential persistence karşılaştırmasıdır. Container ile katalog metadata/active pointer ayrı dosyalarda kaldığında atomik rename, iki state'i tek commit yapmaz. Bu nedenle cross-file metadata atomicity production blocker'ıdır. Gelecekte tercih edilen production yönü wrapped key-state, ciphertext row'ları, snapshot metadata ve active pointer'ı **aynı SQLite transaction domain'inde** tutan `staging/encrypt-only → active/decrypt-only → retired/destroyed` state machine'dir; current no-database container prototipi bu tasarım değildir [S102]. Source credential ve remote-playlist secret'ları mevcut düşük-cardinality owner-bound DPAPI store'da kalır.

Source delete admission-close + lease drain + wrapped-key erişimsizleştirme + bounded ciphertext cleanup + startup reconciliation gerektirir. Shared snapshot key dururken tek channel delete cryptographic erase değildir; journal/free-page/backup/storage kopyalarının tamamı kanıtlanmadan “NIST secure erase” iddiası kurulmaz [S101]. Exact format, failure matrix, comparative ölçüm ve production selection [M4 protected catalog aday kararında](../quality/M4_PROTECTED_CATALOG_CANDIDATE.md) dondurulmuştur.

Worktree'de ayrı test-only `IptvSuite.ProtectedCatalogSpike` implementation'ı bulunur; production/normal workflow'dan architecture ve package denylist ile ayrılır. Exact-hash bağlı dirty-worktree local 1k Smoke correctness/harness zinciri ile targeted architecture 14/14 geçmiştir. Aynı worktree'deki full gate, exact SDK ile Debug+Release x64 build'leri ve architecture 14 + unit 151 + integration 34 = 199/199 testi iki aynı `Passed` setiyle; fixture, sentinel, scanner ve artifact-canary kontrolleriyle geçmiştir. Quality summary `commitSha=null` taşıdığı için bu local sonuç commit-bound değildir; ayrıntı aday karar belgesindedir. Heavy 5k/10k/20k/50k × 20 `Decision` koşusu fresh explicit approval olmadan başlatılmaz ve hâlâ `UNVERIFIED`dır. Bu sonuç M4'ü bitirmez, ADR'yi `Accepted` yapmaz ve production implementation seçmez.
...
- file-per-record DPAPI için tamamlanan 5k/10k/20k/50k Decision benchmark'ından sonra test-only immutable-container adayında aynı allocation, working-set ve cancellation matrisi; heavy `Decision` için fresh explicit approval;
