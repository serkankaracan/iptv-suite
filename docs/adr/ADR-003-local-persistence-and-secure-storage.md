...
Bu ara sonuç ADR'yi `Accepted` yapmaz. Source-draft operation'ı yalnız doğrudan await edilen çağrıda store create sonrası reference'ı başarılı sonuçla döndürür. Tarihsel same-version packaged harness, signed `0.0.1.0 → 0.0.2.0` disposable test-family update lane'i ve schema-v3 disposable reset/live-state uninstall-reinstall fresh-state lane'inin hosted geçişleri workflow/UI düzeyinde tamamlanmıştır; indirilen schema v2/v3 artifact içerikleri bağımsız doğrulanmamıştır. Production PFN lifecycle, repair ve wrong-user'a ek olarak durable authoritative owner metadata'sı, owner + reference + endpoint tuple swap ve M5 resolved-origin politikası, source-wide delete/startup protected-record orphan reconciliation ile deletion/in-flight coordination henüz yoktur. Path/reparse check-to-use yarışı 2026-08-14 threat-model kararında yalnız yükseltilmemiş, kullanıcıya özel local store ve same-user compromise kapsamı için residual risk olarak **ACCEPTED**dır; paylaşılan/ağ store, elevation veya same-user adversary scope değişikliği kararı yeniden açar. Atomik kapatma küçük bir handle patch'i değil, root-relative tam Windows I/O adapter'ı gerektirir [S94].
