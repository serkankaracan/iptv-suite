...
M4'ün ayrı nonpublishable package-lifecycle hostu, production application graph'ına test mode eklemeden aynı disposable test identity/version ve CurrentUser altında iki process launch uygular. DPAPI-protected control ticket source/configuration-owner/reference bağını process sınırından geçirir; create sonrası exact `ProtectedStore` ve lifecycle run-directory canary scan'i, sonraki launchta correct read, cross-owner negative, update/read/delete ve post-delete unavailable davranışı ile exact uninstall/app-data/certificate/output cleanup'ı hedefler. Package root'taki Windows-managed mutable registry hive'ları owned-write-surface taramasına dahil değildir. Harness package/output'u artifact değildir; yalnız sanitized boolean/hash evidence upload edilebilir. Bu bir signed package update, reset/repair, uninstall-reinstall retention, production PFN, second-user veya crash-recovery testi değildir. Kod/workflow tanımı PASS sayılmaz; ilk commit-bound hosted sonucu pending'dir.
