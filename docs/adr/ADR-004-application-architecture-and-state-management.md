...
- M12 — **PARTIAL VERIFIED, 2026-08-25:** Application power policy'si yalnız suspend `Entering` durumunda exact current session'ı stop/drain eder; resume autoplay yapmaz. Exact source release replacement session'ı korur. Run `32764149667`/commit `8d537010f64193f7bc9baf05bc3707fb31d5362f`, 25 rapid switch ile controls/fullscreen/focus/resize/minimize/state/active-close otomasyonunu geçti. Run `32785306860` kısa-run resource budget'ını, run `32792088083`/commit `4e96b27e1397c10df82b63531a3c3f227fe0f774` ise production source-delete cancel/dialog-close no-mutation, active-playback drain, restart non-admission, exact katalog/DPAPI cleanup, completed tombstone binding ve sibling retention zincirini geçti. Fiziksel DPI/multi-monitor/sleep/audio/Narrator `NOT RUN`dır. [Kanıt](../quality/M12_COMPLETION_EVIDENCE.md).
