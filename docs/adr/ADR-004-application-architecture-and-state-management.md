...
- M13 — **AUTOMATED/DETERMINISTIC MVP ACCEPTANCE COMPLETED, 2026-08-25:** Signed production package, controlled Live HLS üzerinde clean EOF → canonical `StreamInterrupted/BoundedTransient` → exact bounded reconnect → current physical `Playing` recovery zincirini; ikinci clean EOF sonrasında erişilebilir user cancel'i `22,087 ms ≤ 1000 ms` içinde ve `31.009 ms` boyunca yeni open olmadan tamamladı. Fault stream iki expected clean completion ve üçüncü held request için bir client detach ile exact `1→2→3` ordinal zincirinde kaldı; expected abort, capacity reject, unexpected failure ve overlap sıfırdır. Pre-final stop yasak, final stop zorunlu ve her durumda strict signal validation altındadır. Commit `90198c0595293cc6ed265f462f1c2cbb3c3c647a` için [run #211 (`32839777516`)](https://github.com/serkankaracan/iptv-suite/actions/runs/32839777516), `553/553 × 2` deterministic quality, signed packaged, DPAPI ve required gate işlerini geçti. Opaque `MediaSource` üzerinde exact HTTP/TLS/DNS attribution, gerçek provider/cihaz ve long-soak sonucu iddia edilmez. [Completion evidence](../quality/M13_COMPLETION_EVIDENCE.md).
