...
- M12 — **PARTIAL VERIFIED, 2026-08-24:** Application power policy'si yalnız suspend `Entering` durumunda exact current session'ı stop/drain eder; resume autoplay yapmaz. Exact source release replacement session'ı korur. Run `32764149667`/commit `8d537010f64193f7bc9baf05bc3707fb31d5362f`, 25 rapid switch ile controls/fullscreen/focus/resize/minimize/state/active-close otomasyonunu geçti. Tek resource snapshot numeric budget PASS değildir; fiziksel DPI/multi-monitor/sleep/audio/Narrator ve product source-delete E2E `NOT RUN`dır. [Kanıt](../quality/M12_COMPLETION_EVIDENCE.md).
- M13 property/state transition ve retry/reconnect cancel race testleri.
