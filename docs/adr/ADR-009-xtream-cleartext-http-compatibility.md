...
- Account discovery, aynı exact `player_api.php` origin'inde ve aynı response/time/redirect/header politikasıyla en fazla üç internal action varyantını `get_account_info → action yok → get_profile` sırasıyla dener. Fallback yalnız başarılı HTTP response'un body shape'i unsupported ise ilerler; explicit authentication rejection, redirect/HTTP failure, timeout, rate-limit, service failure ve cancellation tekrar edilmez. HTTP source'ta credential'ın aynı origin'e üç kez cleartext gönderilebilmesi [uyumluluk ödünleri kaydında](../security/COMPATIBILITY_CONCESSIONS.md) `CC-005` olarak görünür kalır.
- Provider JSON'u UTF-8 BOM ile başlayabilir; BOM yalnız tek başlangıç preamble'ı olarak atılır. Live/VOD/Series koleksiyonu canonical array'e ek olarak yalnız bounded ve benzersiz sayısal anahtarlara, object/null değerlere sahip associative object biçiminde kabul edilir.
- Başarılı account doğrulamasından sonraki catalog zincirinde exact boş object, top-level `null` ve `false`, provider'ın bulunmayan içerik ailesini bildiren boş sentinel'i olarak kabul edilir. Non-empty error object, `true`, number, string, HTML, duplicate/mixed associative object ve malformed JSON reddedilir. Bu tolerans ilk importta provider outage/error durumunu gerçek boş aileden ayıramaz; veri bütünlüğü/availability ödünü [uyumluluk ödünleri kaydında](../security/COMPATIBILITY_CONCESSIONS.md) `CC-004` olarak aktiftir. Partial/empty warning ayrı product/milestone kararıdır.
- Existing non-empty Live/Movie/Series ailesi sentinel veya tamamı geçersiz item sayfasıyla sıfıra düşecekse yeni snapshot aktive edilmez ve last-good snapshot korunur. Canonical explicit `[]` authoritative empty beyanı sayılır; hatalı bir explicit empty provider response'u yine içeriği silebilir. İlk importta karşılaştırılacak last-good olmadığı için sentinel empty kabul edilir.
- Live/VOD display name `name → title`, Series `title → name` bounded string precedence'i kullanır; number/bool display name reddedilir. Series detail'de `seasons` missing/null olabilir, fakat `episodes` bounded object zorunludur. Raw season-group ve raw episode sayısı sırasıyla `100/5.000` hard cap'tir; invalid item'lar bu bütçeyi düşürmez. Bu proprietary tolerans `CC-006` altında izlenir.
- Category/Live/VOD/Series item bütçesi aşıldığında sonuç generic format hatasına indirgenmez; `PlaylistEntryLimitExceeded` olarak görünür olur. Unexpected response shape ise account, Live, Movies veya Series aşamasına ait ayrı stable safe error'a çevrilir. Bu hata hiçbir response body, URL, action query'si, username veya password taşımaz.
...
- Provider: bounded account action fallback matrisi; sentetik Live/VOD/Series variants; string-only `name/title` precedence'i; missing Series season metadata; raw season/episode budget; UTF-8 BOM, bounded unique numeric-key collection ve exact empty/null/false sentinel compatibility; last-good anti-empty activation; non-empty error/mixed object ile malformed/deep/oversized/duplicate catalog fail-closed sonuçları; item-limit ve safe-stage error propagation; heuristic content typing yasağı.
...
- [Compatibility Concessions](../security/COMPATIBILITY_CONCESSIONS.md)
