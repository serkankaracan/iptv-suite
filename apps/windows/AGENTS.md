...
- `SafeEndpoint` yalnız normalized `http`/`https` scheme, IDNA host ve effective port taşır; raw path, query, fragment veya user-info'dan locator üretmez. Xtream-compatible source yalnız HTTPS kalır; HTTP yalnız ADR-008 kapsamındaki açık-onaylı Remote M3U source için açılabilir.
- Remote M3U HTTP locator query/token taşıyabilir fakat user-info taşıyamaz. Initial HTTP channel locator yalnız HTTP source'un exact `SafeEndpoint` origin'iyle aynıysa kabul edilir; HTTPS channel initial URI/scheme validation ile kabul edilir. HTTP logo, Xtream HTTP, HTTP request'te `Authorization`/`Cookie`/`Referer`, cross-origin HTTP playlist redirect ve HTTPS→HTTP downgrade yasaktır; revalidated HTTP→HTTPS upgrade mümkündür. Native `MediaSource` HLS/media alt-kaynakları ile player-internal redirect'lerinde same-origin veya `EndpointAddressPolicy` enforcement varmış gibi kod, test ya da belge iddiası oluşturma; bu yüzey `UNVERIFIED` release blocker'dır.
- HTTP source ağ isteğinden önce scheme-specific UI onayı ve source yaşam döngüsü boyunca kalıcı insecure-transport uyarısı ister. Protected-at-rest locator'ın transit confidentiality/integrity sağlamadığını gizleme veya HTTP'yi güvenli diye etiketleme.
- Remote M3U indirmesi scheme'den bağımsız olarak decompressed response için `128 MiB`, toplam request için iki dakika ile sınırlıdır; genel HTTP/Xtream yolu `4 MiB` ve varsayılan 15 saniye sınırını korur. Remote M3U onboarding, HTTP/HTTPS ayrımı olmadan preliminary full-body probe yapmamalı; validation — HTTP için ayrıca açık onay — sonrasında protected locator'ı stage edip tek bounded streaming import isteği kullanmalı, açıkça `NotCommitted` sonuçta exact protected kaydı temizlemelidir.
