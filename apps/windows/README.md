...
- Normal MSTest process'i/temp-root kanıtına ek olarak ayrı `IptvSuite.PackageLifecycleHarness`, aynı disposable test package ve kullanıcı bağlamında üç ardışık packaged activation ile owner-bound v2 create/duplicate-create rejection/read/wrong-owner/update/delete/post-delete zincirini çalıştırır. Bu harness production payload'a girmez. Package update/reset/reinstall, production PFN, gerçek ikinci Windows user, source-wide deletion/reconciliation, 5k–50k layout/performance ve ADR-003 final kararı açık hard-gate'tir.
...
| Packaged lifecycle | Ayrı test MSIX'inde üç ardışık process activation ile CurrentUser DPAPI v2 create/duplicate-create rejection/read/wrong-owner/update/delete/post-delete, owned-surface canary scan ve exact cleanup | Production PFN, package version update/reset/reinstall, second-user veya source-wide reconciliation |
